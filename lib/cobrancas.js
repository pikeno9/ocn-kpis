...
// Semana 0 (retirada) e semanas abonadas ficam de fora (não são receita).
  ...
  for (const p of (d && d.placas) || []) {
    ...
    for (const v of p.vinculos || []) {
      for (const s of v.semanas_pagas || []) {
        if (s.semana === 0) continue; // retirada = entrega do carro, não cobrança
        if (s.tipo !== 'pago' && s.tipo !== 'pago_atrasado') continue; // abonado / não pago: sem receita
