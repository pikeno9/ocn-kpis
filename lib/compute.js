...
// cliente divide pelo total de carros das duas. Vencimento vazio (1ª parcela de endosso) é
// inferido: um mês antes da parcela seguinte, seguindo o padrão mensal do dia 10.
function parseInsurancePay(rows) {
  ...
  // vencimento faltante: um mês antes da parcela seguinte da MESMA série
  Object.values(porFrota).forEach((list) => {
    list.forEach((p) => {
      if (p.venc) return;
      const next = list.find((q) => q.ref === p.ref && q.parcela === p.parcela + 1 && q.venc);
      if (next) { const d = new Date(next.venc + 'T12:00:00'); d.setMonth(d.getMonth() - 1); p.venc = d.toISOString().slice(0, 10); }
    });
  });
