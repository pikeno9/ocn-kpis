...
// ----------------- REDEPLOYMENT: Recuperações (aba import_Time) -----------------
// Bloco "Tempo para recolocação - Recuperações": uma linha de cabeçalho combina o
// título da seção com os rótulos de coluna; localiza tudo pelo TEXTO do cabeçalho
// (não por posição fixa) para não quebrar se colunas forem reordenadas/inseridas.
...
function parseRecuperacoes(rows) {
  if (!rows || !rows.length) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  let headerRow = -1;
  for (let i = 0; i < rows.length; i++) {
  ...
  const cols = {
    ...
    dataRecup: findCol(/data da recupera/i),
  ...
  if (cols.dataRecup < 0 || cols.placa < 0) return null;

  const items = [];
  for (let i = headerRow + 1; i < rows.length; i++) {
    if (!cv(rows[i], cols.placa)) break; // fim da seção (linha em branco antes do próximo bloco)
    const dtRecup = parseDataUS(cv(rows[i], cols.dataRecup));
    if (!dtRecup) continue;
    const dp = cols.diasPronto >= 0 ? cv(rows[i], cols.diasPronto) : '';
    const da = cols.diasAlocado >= 0 ? cv(rows[i], cols.diasAlocado) : '';
    items.push({
      placa: cv(rows[i], cols.placa),
      cliente: cv(rows[i], cols.cliente),
      dataRecuperacao: dtRecup.toISOString().slice(0, 10),
      dataPronto: cols.dataPronto >= 0 ? cv(rows[i], cols.dataPronto) : '',
      dataRecolocacao: cols.dataRecoloc >= 0 ? cv(rows[i], cols.dataRecoloc) : '',
      motivo: cols.motivo >= 0 ? cv(rows[i], cols.motivo) : '',
      detalhamento: cols.detalhe >= 0 ? cv(rows[i], cols.detalhe) : '',
      diasRecupParaPronto: dp === '' ? null : (parseFloat(dp) || 0),
      diasProntoParaAlocado: da === '' ? null : (parseFloat(da) || 0),
      monthKey: dtRecup.getFullYear() + '-' + String(dtRecup.getMonth() + 1).padStart(2, '0'),
    });
  }
  ...
  return {
    ...
    detail: monthKeys.map((k) => byMonth[k].items.map((it) => ({
      ...
      dataRecuperacao: dmy(it.dataRecuperacao),
...
function build(sheets, referenceDate) {
  ...
  return {
    ...
    redeployment: { recoveries: parseRecuperacoes(sheets.time) },
    kpis: {
