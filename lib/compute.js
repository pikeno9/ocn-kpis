...
function parseTimeSections(rows) {
  ...
  return {
    recoveries: summarize(readItems(rowRecuperacoes)),
    returns: rowDevolucoes >= 0 ? summarize(readItems(rowDevolucoes + 1)) : null,
    swaps: rowTroca >= 0 ? summarize(readItems(rowTroca + 1)) : null,
  };
}
...
function build(sheets, referenceDate) {
  ...
  // série mensal de ocorrências (col D = Data do Evento) — linhas sem data ficam de fora
  // e são contadas em semData (o card mostra a nota até a planilha ser completada)
  const occMesMap = {};
  let occSemData = 0;
  for (const r of occ) {
  ...
  const occMensal = { labels: [], monthKeys: [], values: [], semData: occSemData };
  if (occKeys.length) {
  ...
  return {
    ...
    redeployment: parseTimeSections(sheets.time) || { recoveries: null, returns: null, swaps: null },
    kpis: {
