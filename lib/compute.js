...
// Nova base tidy da inDrive (aba "import_indrive(dados)"): tabela semanal em colunas nomeadas
// — semana | total_ativos_ocn | inelegiveis | capturados | elegiveis_nao_capturados.
// Substitui parsePerfInDrive como fonte do 1º gráfico (barras empilhadas), MESMO formato/campos.
// Robusto: exige os cabeçalhos próprios; se o gviz devolver outra aba, retorna null (cai no fallback).
function parsePerfInDriveData(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  const header = rows[0] || [];
  const col = (re) => header.findIndex((c) => re.test(String(c || '').trim()));
  const cSem = col(/^semana$/i), cAtivos = col(/^total_ativos_ocn$/i), cInel = col(/^inelegiveis$/i);
  const cCap = col(/^capturados$/i), cElegNC = col(/^elegiveis_nao_capturados$/i);
  if (cSem < 0 || cAtivos < 0 || cCap < 0 || cElegNC < 0) return null;
  const num = (s) => { const n = parseInt(String(s).replace(/[^\d-]/g, ''), 10); return isFinite(n) ? n : 0; };
  const weeks = [];
  for (let i = 1; i < rows.length; i++) {
    const sem = cv(rows[i], cSem);
    const m = sem.match(/^(\d{1,2})\/(\d{1,2})\/(\d{2,4})$/); // 12/06/2026 (DD/MM/YYYY)
    if (!m) continue;
    const ativos = num(cv(rows[i], cAtivos));
    if (!ativos) continue;
    const cap = num(cv(rows[i], cCap)), elegNC = num(cv(rows[i], cElegNC)), inel = num(cv(rows[i], cInel));
    const eleg = cap + elegNC; // elegíveis = capturados + elegíveis não capturados
    weeks.push({
      label: m[1].padStart(2, '0') + '/' + m[2].padStart(2, '0'),
      full: sem, ativos, eleg, prints: cap, naoEleg: inel,
      pctEleg: ativos ? Math.round((eleg / ativos) * 100) : 0,
      pctCaptura: ativos ? Math.round((cap / ativos) * 100) : 0,
    });
  }
  if (!weeks.length) return null;
  return {
...
// ----------------- REVISÕES REALIZADAS (aba import_rev) -----------------
// Uma linha por revisão efetivamente feita: B=Placa, D=Revisão referência (10000, 20000…),
// E=KM na revisão, F=Data agendada, K=Valor, L=Data de vencimento do pagamento, S=Pago?.
// Linha SEM valor/vencimento = revisão ocorreu mas ainda não temos os dados de pagamento.
// `prazoMedioDias` = média de (vencimento − data agendada) das que já têm os dois — usada
// para projetar quando as demais revisões serão pagas.
function parseRevBase(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  const dt = (s) => { const m = String(s || '').match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/); return m ? `${m[3]}-${m[2].padStart(2, '0')}-${m[1].padStart(2, '0')}` : null; };
  // Esta aba grava os valores em formato US ("617.00", "671.18") — diferente do resto da planilha,
  // que é pt-BR. Detecta pelo separador: com os dois, o último manda; só vírgula = decimal pt-BR.
  const num = (s) => {
    let t = String(s == null ? '' : s).trim().replace(/[R$\s]/gi, '');
    if (!t) return null;
    const hasDot = t.includes('.'), hasCom = t.includes(',');
    if (hasDot && hasCom) t = (t.lastIndexOf(',') > t.lastIndexOf('.')) ? t.replace(/\./g, '').replace(',', '.') : t.replace(/,/g, '');
    else if (hasCom) t = t.replace(',', '.');
  ...
  const prazos = [];
  for (let i = 1; i < rows.length; i++) {
    const pl = cv(rows[i], 1).toUpperCase();
    if (!pl) continue;
    const refKm = num(cv(rows[i], 3));
    const n = refKm ? Math.round(refKm / 10000) : null;
    if (!n) continue;
    const data = dt(cv(rows[i], 5));           // F — data agendada da revisão
    const venc = dt(cv(rows[i], 11));          // L — vencimento do pagamento
    const valor = num(cv(rows[i], 10));        // K — valor
    if (data && venc) prazos.push((new Date(venc) - new Date(data)) / 86400000);
    (placas[pl] = placas[pl] || []).push({
      n, km: num(cv(rows[i], 4)), data, valor, venc,
      pago: /^sim$/i.test(cv(rows[i], 18)),
    });
  }
  if (!Object.keys(placas).length) return null;
  const prazoMedioDias = prazos.length ? Math.round(prazos.reduce((a, b) => a + b, 0) / prazos.length) : 33;
  return { placas, prazoMedioDias, total: prazos.length };
}

// ----------------- MULTAS A PAGAR (aba multas_consolidado) -----------------
// O que a OCN DEVE pagar (lado da despesa; a receita vem da API de cobranças).
// F=placa, D=data do e-mail (quando a multa chegou), K=data/hora da infração,
// U=valor a pagar LM, W=vencimento (quando pagamos). Só ~metade tem vencimento.
// `prazoMedioDias` = média de (vencimento − data do e-mail) entre as que já têm data —
// usada para estimar quando as demais serão pagas. Escolhi o e-mail como âncora (e não a
// data da infração) porque o pagamento só é possível depois que a notificação chega: esse
// prazo é bem mais estável (25–56 dias) que o da infração (41–105).
function parseMultasBase(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  const dt = (s) => { const m = String(s || '').match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/); return m ? `${m[3]}-${m[2].padStart(2, '0')}-${m[1].padStart(2, '0')}` : null; };
  const num = (s) => {
  ...
  const prazos = [];
  for (let i = 1; i < rows.length; i++) {
    const pl = cv(rows[i], 5).toUpperCase();
    if (!pl) continue;
    const v = num(cv(rows[i], 20));           // U — valor a pagar LM
    if (v == null) continue;
    // N = valor bruto da multa · O = valor com desconto (líquido, ~80% do bruto).
    // A regra do contrato usa OS DOIS: cobramos do cliente o BRUTO + prêmio (10% v1/v2, 20% v3+)
    // e pagamos à LM o LÍQUIDO + 5% — que é exatamente o que a coluna U traz (conferido: U = O × 1,05).
    const bruto = num(cv(rows[i], 13));       // N — valor_bruto_multa
    const liq = num(cv(rows[i], 14));         // O — valor_com_desconto
    const email = dt(cv(rows[i], 3));         // D — chegada da notificação
    const inf = dt(cv(rows[i], 10));          // K — data da infração
    const venc = dt(cv(rows[i], 22));         // W — vencimento do nosso pagamento
    if (venc && email) prazos.push((new Date(venc) - new Date(email)) / 86400000);
    (placas[pl] = placas[pl] || []).push({ v, bruto, liq, email, inf, venc });
  }
  if (!Object.keys(placas).length) return null;
  const prazoMedioDias = prazos.length ? Math.round(prazos.reduce((a, b) => a + b, 0) / prazos.length) : 44;
  // desconto médio efetivo (líquido ÷ bruto) — usado quando uma multa não traz as duas colunas
  let sb = 0, sl = 0;
  Object.values(placas).forEach((a) => a.forEach((x) => { if (x.bruto > 0 && x.liq > 0) { sb += x.bruto; sl += x.liq; } }));
  const descontoMedio = sb > 0 ? sl / sb : 0.8;
  return { placas, prazoMedioDias, comVenc: prazos.length, descontoMedio };
}

// ----------------- JUDICIAL (aba import_jud) -----------------
// Casos de recuperação/rescisão: I=Placa, F=Tipo do evento, K=Cobrança total,
// N=Infrações de trânsito, O=Pedágio+free flow, P=Guincho, Q=Custo de recuperação,
// R=Reparo de avarias, S=Higienização, T=Outros, U=Data do evento.
// Por caso: recovery = P+Q (guincho + recuperação), repair = R+S+T,
// term = K−N−O (a cobrança total sem as parcelas que já entram em Traffic fines).
function parseJudBase(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  const dt = (s) => { const m = String(s || '').match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/); return m ? `${m[3]}-${m[2].padStart(2, '0')}-${m[1].padStart(2, '0')}` : null; };
  const num = (s) => {
  ...
  let casos = 0;
  for (let i = 1; i < rows.length; i++) {
    const pl = cv(rows[i], 8).toUpperCase();
    if (!pl || !/^[A-Z]{3}\d[A-Z0-9]\d{2}$/.test(pl)) continue;
    const guincho = num(cv(rows[i], 15)), recup = num(cv(rows[i], 16)); // P guincho · Q recuperação
    const recovery = guincho + recup;
    const repair = num(cv(rows[i], 17)) + num(cv(rows[i], 18)) + num(cv(rows[i], 19)); // R+S+T
    const term = Math.max(0, num(cv(rows[i], 10)) - num(cv(rows[i], 13)) - num(cv(rows[i], 14))); // K−N−O
    if (!recovery && !repair && !term) continue;
    casos++;
    (placas[pl] = placas[pl] || []).push({ d: dt(cv(rows[i], 20)), tipo: cv(rows[i], 5), recovery, guincho, recup, repair, term });
  }
  return casos ? { placas, casos } : null;
}

// ----------------- SEGURO REAL (aba insurance) -----------------
// Parcelas efetivas do seguro por frota: referente (Apólice/Endosso), nº da parcela, valor e
// vencimento. A frota 5 não tem linhas próprias — o endosso dela vive dentro da frota 4, e o
// cliente divide pelo total de carros das duas. Vencimento VAZIO = parcela paga à vista na
// assinatura (fica null aqui; o cliente usa a data de início da frota). Inferir "um mês antes
// da parcela seguinte" jogava a 1ª parcela da frota 6 em setembro e inflava o mês em ~10k.
function parseInsurancePay(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  // O gviz ora devolve "31.413,50" (pt-BR) ora "31,413.50" (EN) para a MESMA célula, conforme
  // o humor da inferência de tipo. O separador decimal é decidido pelo ÚLTIMO símbolo presente —
  // assumir pt-BR fixo fazia a leitura EN sair 1000× menor.
  const num = (s) => {
    let t = String(s == null ? '' : s).trim().replace(/[R$\s]/gi, '');
    if (!t) return 0;
    const lc = t.lastIndexOf(','), ld = t.lastIndexOf('.');
    if (lc > -1 && ld > -1) t = lc > ld ? t.replace(/\./g, '').replace(',', '.') : t.replace(/,/g, '');
    else if (lc > -1) t = t.replace(/\./g, '').replace(',', '.');
    const v = parseFloat(t); return isFinite(v) ? v : 0;
  };
  const dt = (s) => { const m = String(s || '').match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/); return m ? `${m[3]}-${m[2].padStart(2, '0')}-${m[1].padStart(2, '0')}` : null; };
  const porFrota = {};
  for (let i = 1; i < rows.length; i++) {
    const frota = cv(rows[i], 4);
    const valor = num(cv(rows[i], 2));
    if (!frota || !(valor > 0)) continue;
    (porFrota[frota] = porFrota[frota] || []).push({ ref: cv(rows[i], 0), parcela: +cv(rows[i], 1) || 0, valor, venc: dt(cv(rows[i], 3)) });
  }
  return Object.keys(porFrota).length ? { porFrota } : null;
}
...
function build(sheets, referenceDate) {
  ...
  // O acumulado é de carros TOTAIS na frota: mês sem recebimento repete a frota anterior até o
  // MÊS VIGENTE — jul/ago ficavam vazios só porque nenhum carro novo chegou depois de junho.
  lastMonth = Math.max(lastMonth, Math.min(8, Math.max(0, today.getMonth() + 1 - 4)));
  const acumRecebido = {};
  ...
  for (const r of occ) {
    ...
    const sin = cell(r, 4);
    const hasSin = (sin && sin.toUpperCase() !== 'N/A') || (it && it.label === 'Window Damage'); // Window Damage sempre tem insurance claim
    const ofi = cell(r, 7);
  ...
  // Dono do carro na DATA do evento: cruza placa + janela do vínculo (import_clientes).
  // A coluna "Cliente" da aba Ocorrencias costuma vir vazia/errada, então priorizamos aqui.
  const vincByPlaca = {};
  for (const r of sheets.clientes) {
    const placa = cell(r, 6).toUpperCase(); // col G = Placa
    const nome = cell(r, 1);                // col B = Nome Motorista
    if (!placa || !nome) continue;
    const start = parseFlex(cell(r, 9), 'MD');                     // col J = Data Início Vínculo
    const end = cell(r, 10) ? parseFlex(cell(r, 10), 'DM') : null; // col K = Data Fim Vínculo (vazio = ainda ativo)
    (vincByPlaca[placa] = vincByPlaca[placa] || []).push({ nome, start, end });
  }
  const clienteNaData = (placa, d) => {
    const vs = vincByPlaca[String(placa || '').toUpperCase()] || [];
    if (d) { for (const v of vs) { if (v.start && d >= v.start && (!v.end || d <= v.end)) return v.nome; } }
    if (vs.length === 1) return vs[0].nome; // placa com um único dono: alta confiança mesmo sem data no intervalo
    return null;
  };
  const occCases = occ.map((r) => {
    ...
    return {
      ...
      placa: cell(r, 1) || '—',
      cliente: clienteNaData(cell(r, 1), d) || ((cell(r, 2) && cell(r, 2) !== '—') ? cell(r, 2) : '—'), // dono na data (import_clientes); fallback: coluna Cliente de Ocorrencias

      tipo: it ? it.label : (cell(r, 8) || '—'),
      tipoCor: it ? it.cor : '#6B7280',
      oficina: hasOfi ? ofi : '—',
      sinistro: !!((sin && sin.toUpperCase() !== 'N/A') || (it && it.label === 'Window Damage')), // Window Damage sempre tem insurance claim
      sinistroId: (sin && sin.toUpperCase() !== 'N/A' && sin !== '-' && sin !== '–') ? sin : ((it && it.label === 'Window Damage') ? 'N.A' : '—'), // Window Damage: tem claim mas sem ID -> "N.A"
  ...
  return {
    ...
    // perf (1º gráfico, barras empilhadas) agora vem da base nova "import_indrive(dados)";
    // fallback para a aba antiga "Performance inDrive" se a nova não estiver disponível.
    inDrive: { leads: parseLeadsInDrive(sheets.leadsInDrive), perf: parsePerfInDriveData(sheets.perfInDriveData) || parsePerfInDrive(sheets.perfInDrive) },
...
// ----------------- PROMOÇÃO INDRIVE (aba import_baseID) -----------------
// Uma linha por motorista da fase de ativação. Interessa o desconto na semanalidade CREDITADO
// (col M) por placa (col E) — só linhas com desconto_creditado = "Sim" (col K). Cada placa
// aparece no máximo uma vez com desconto creditado (conferido na base: 87 placas, sem repetição).
function parseBaseID(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  const num = (s) => {
    let t = String(s == null ? '' : s).trim().replace(/[R$\s]/gi, '');
  ...
  const descontos = {};
  const rodadas = {};   // placa -> rodada de ativacao (col Q). A 1a rodada nao tem lancamento datado no painel de cobrancas.
  for (let i = 0; i < rows.length; i++) {
    const placa = cv(rows[i], 4).toUpperCase();
    if (!/^[A-Z]{3}\d[A-Z0-9]\d{2}$/.test(placa)) continue;           // pula título/cabeçalho
    if (!/^sim$/i.test(cv(rows[i], 10))) continue;                    // só desconto CREDITADO
    const v = num(cv(rows[i], 12));
    if (v > 0) descontos[placa] = (descontos[placa] || 0) + v;
    const rd = cv(rows[i], 16); if (rd) rodadas[placa] = String(rd).trim();
  }
  return Object.keys(descontos).length ? { descontos, rodadas } : null;
}

// ----------------- INDRIVE (aba "indrive") -----------------
// Tabela única da promoção, uma linha por placa: valor e DATA do desconto creditado e do bônus
// recebido. Antes o bônus vinha de uma tabela fixa no código e o desconto da aba import_baseID,
// os dois sem data — tudo caía no M0 do UE. Com a data, cada lançamento cai no mês de vida do
// carro em que o dinheiro realmente andou. Linha sem data continua caindo no M0.
// Colunas (pela ORDEM, com cabeçalho na 1ª linha):
//   A placa · B valor_desconto · C data_desconto · D valor_bonus · E data_bonus · F rodada
function parseIndrive(rows) {
  if (!rows || rows.length < 2) return null;
  const cv = (r, i) => ((r && r[i] != null) ? String(r[i]).trim() : '');
  // O gviz serve a PRIMEIRA aba quando o nome pedido não existe (não dá 404). Sem conferir o
  // cabeçalho, a aba do funil entraria aqui como se fosse a da promoção. Só segue se a primeira
  // linha for mesmo o cabeçalho esperado.
  const head = (rows[0] || []).map((x) => String(x || '').trim().toLowerCase());
  if (!(head[0] === 'placa' && /valor[_ ]?desconto/.test(head[1] || ''))) return null;
  const num = (s2) => {
    let t = String(s2 == null ? '' : s2).trim().replace(/[R$\s]/gi, '');
    if (!t) return 0;
    const lc = t.lastIndexOf(','), ld = t.lastIndexOf('.');
    if (lc > -1 && ld > -1) t = lc > ld ? t.replace(/\./g, '').replace(',', '.') : t.replace(/,/g, '');
    else if (lc > -1) t = t.replace(/\./g, '').replace(',', '.');
  ...
  // aceita dd/mm/aaaa e aaaa-mm-dd (a coluna pode estar como texto ou como data)
  const dt = (s2) => {
  ...
  const placas = {};
  for (let i = 1; i < rows.length; i++) {
    const pl = cv(rows[i], 0).toUpperCase();
    if (!/^[A-Z]{3}\d[A-Z0-9]\d{2}$/.test(pl)) continue;   // pula cabeçalho e linhas soltas
    const o = placas[pl] || (placas[pl] = { desconto: 0, descontoEm: null, bonus: 0, bonusEm: null });
    const vd = num(cv(rows[i], 1)), vb = num(cv(rows[i], 3));
    if (vd > 0) { o.desconto += vd; o.descontoEm = o.descontoEm || dt(cv(rows[i], 2)); }
    if (vb > 0) { o.bonus += vb; o.bonusEm = o.bonusEm || dt(cv(rows[i], 4)); }
    const rd = cv(rows[i], 5); if (rd) o.rodada = rd;
  }
  return Object.keys(placas).length ? { placas } : null;
}
module.exports = { build, parseRevBase, parseMultasBase, parseJudBase, parseInsurancePay, parseBaseID, parseIndrive };
