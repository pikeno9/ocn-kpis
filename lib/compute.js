...
function parseJudBase(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const recovery = num(cv(rows[i], 15)) + num(cv(rows[i], 16));       // P guincho + Q recuperação
    ...
    casos++;
    (placas[pl] = placas[pl] || []).push({ d: dt(cv(rows[i], 20)), tipo: cv(rows[i], 5), recovery, repair, term });
  }
