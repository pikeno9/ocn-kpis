...
async function fetchKmSemanal(plates) {
  ...
  const fetchOne = async (plate) => {
    try {
      // rate limit da API: 429 espera e tenta de novo (até 3x com backoff crescente)
      let r = null;
      for (let tent = 0; tent < 3; tent++) {
        r = await fetch(C.FROTA_API + '?resource=readings&plate=' + encodeURIComponent(plate) + '&limit=500', { headers: { Authorization: 'Bearer ' + C.FROTA_TOKEN } });
        if (r.status !== 429) break;
        await new Promise((ok) => setTimeout(ok, 1200 * (tent + 1)));
      }
      if (!r || !r.ok) return;
      const d = await r.json();
  ...
  // concorrência CONTIDA: ~170 placas sem estourar o rate limit da API (com 8 deu 429 em cascata)
  const queue = [...plates];
