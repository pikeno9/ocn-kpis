...
async function fetchMultas() {
  ...
  let premVal = 0, premW = 0;            // prêmio médio historicamente aplicado (premio_pct)
  ...
  for (const p of (d && d.placas) || []) {
    ...
    for (const i of p.infracoes || []) {
      ...
      const pp = Number((i.valores || {}).premio_pct);
      if (isFinite(pp)) { premVal += pp * v; premW += v; }
  ...
  const premioMedioHist = premW > 0 ? premVal / premW : 0.10;
  return { placas, prazoRecebimentoDias, taxaRecebimento, premioMedioHist };
}
