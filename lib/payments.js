...
function build(matrizData, clientesRows, refDate, esperadoMap) {
  ...
  // ATENÇÃO: o import_clientes ganhou a coluna "Link Admin" (G) e TUDO andou uma casa. Os índices
  // antigos (status 11, motivo 12, data 10, placa 6) passaram a ler data-fim/status/data-início/link,
  // e NENHUM evento de churn era registrado — os contadores recovered/returned ficavam sempre em 0
  // (medido: 0 eventos aqui contra 21 recuperações + 8 rescisões na planilha).
  // Layout atual: H=7 placa, K=10 início, L=11 fim, M=12 status, N=13 motivo.
  for (const r of clientesRows || []) {
    if (cell(r, 12).toLowerCase() !== 'inativo') continue; // só vínculos encerrados
    const cat = churnCategory(cell(r, 13));
    if (!cat) continue;
    const dt = parseDataBR(cell(r, 11)); // col L = Data Fim Vínculo
    if (!dt) continue;
    ...
    const w = ensure(key);
    w[cat].push({ nome: cell(r, 1) || 'Unknown', placa: cell(r, 7).toUpperCase() });
  }
