...
function build(matrizData, clientesRows, refDate, esperadoMap) {
  ...
  return {
    weeks: dates.map((k) => {
      ...
      // BARRA: Pendente = Esperado(resumo) − resolvido (bate com o site). Nomes/valores da lista vêm do em_aberto.
      const esperado = esp[k] != null ? esp[k] : resolved;
      const pendingCount = Math.max(0, esperado - resolved);
      return {
        date: k,
        esperado,
        ...
        names: w, // names.pending = pessoas em_aberto (nome + valor esperado)
      };
    }),
  };
}
