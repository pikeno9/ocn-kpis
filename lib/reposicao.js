...
async function fetchReposicao() {
  ...
  const placas = {};
  // O campo `subtipo` virou ESTRUTURADO na API — é a fonte primária agora. A classificação por
  // palavra-chave no texto livre fica de fallback: perdia trocas reais cujo detalhe fala em
  // "freios/barulho" sem citar "pastilha" (2 casos auditados em 13/08).
  const SUBTIPO = { pastilhas: 'pastilhas', discos: 'disco', disco: 'disco', pneus: 'pneus', pneu: 'pneus' };
  for (const o of (d && d.ocorrencias) || []) {
    if (o.categoria !== 'itens_reposicao') continue;
    if (o.status === 'cancelado') continue;
    const itens = [];
    const sub = SUBTIPO[String(o.subtipo || '').toLowerCase()];
    if (sub) itens.push(sub);
    else {
    ...
    // classificação OFICIAL natural × atípica quando a API expuser (hoje não existe — o UE usa a
    // heurística de km como fallback). Aceita `classificacao: 'natural'|'atipico'` ou `natural: bool`.
    let natural = null;
    if (typeof o.natural === 'boolean') natural = o.natural;
    else if (o.classificacao) natural = /natural/i.test(String(o.classificacao));
    (placas[o.plate] = placas[o.plate] || []).push({ d: String(o.data_evento).slice(0, 10), itens, natural });
  }
  return { placas };
}
