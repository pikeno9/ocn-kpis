...
async function fetchAllTabs(tabs) {
  const [importData, ocorrencias, clientes, rh] = await Promise.all([
    fetchTab(tabs.importData),
    fetchTab(tabs.ocorrencias),
    fetchTab(tabs.clientes),
    // RH é opcional: falha não derruba o refresh (a seção mostra aviso)
    tabs.rh ? fetchTab(tabs.rh).catch((e) => { console.error(`[rh] aba "${tabs.rh}" falhou:`, e.message); return null; }) : Promise.resolve(null),
  ]);
  return { importData, ocorrencias, clientes, rh };
}
