...
async function fetchAllTabs(tabs) {
  const [importData, ocorrencias, clientes, rh, leads, time, funil, leadsInDrive, perfInDrive, perfInDriveData, rev, multasCons, jud, carrosEsperados, insurance, baseID, indriveTab] = await Promise.all([
    ...
    optional(tabs.perfInDriveData, 'perfInDriveData'),
    optional(tabs.rev, 'rev'),
    optional(tabs.multasCons, 'multasCons'),
    optional(tabs.jud, 'jud'),
    optional(tabs.carrosEsperados, 'carrosEsperados'),
    viaGid(tabs.insurance, TAB_GIDS.insurance).catch(() => null),   // export por gid: preserva vencimento em texto
    optional(tabs.baseID, 'baseID'),
    optional(tabs.indrive, 'indrive'),
  ]);
  return { importData, ocorrencias, clientes, rh, leads, time, funil, leadsInDrive, perfInDrive, perfInDriveData, rev, multasCons, jud, carrosEsperados, insurance, baseID, indriveTab };
}
