...
function buildChurn(clientesRows, fleets, refDate) {
  ...
  for (const r of clientesRows || []) {
    ...
    const rec = inativo && /recupera/i.test(cell(r, 13));    // col N = Motivo Fim Vínculo
    const bump = (o) => {
      o.dias += dias;
...
function buildVinculos(clientesRows) {
  ...
  for (const r of clientesRows || []) {
    ...
    const ini = parseData(cell(r, 10));
    if (!ini || isNaN(ini) || ini.getFullYear() < 2020) continue;   // linhas com data lixo (31/12/1899)
...
function build(ueSheets, importRows, clientesRows, refDate) {
  ...
  return {
    ...
    churn: buildChurn(clientesRows || [], fleets, hoje),
    vinculos: buildVinculos(clientesRows || []),
    orcado,
