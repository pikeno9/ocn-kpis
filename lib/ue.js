...
function buildContratos(clientesRows) {
  ...
  for (const r of clientesRows || []) {
    ...
    if (!placa || cell(r, 11)) continue;      // com data de fim = vínculo encerrado
...
function buildStarts(clientesRows) {
  const out = {};
  for (const r of clientesRows || []) {
    const placa = cell(r, 7).toUpperCase();   // col H = Placa
    if (!placa) continue;
    const d = parseData(cell(r, 10));         // col K = Data Início Vínculo
    if (!d || isNaN(d)) continue;
    const iso = d.toISOString().slice(0, 10);
    if (!out[placa] || iso < out[placa]) out[placa] = iso; // 1º vínculo = entrega do carro
  }
  return out;
}

// Inadimplência medida no histórico: contrato encerrado por "Recuperação" = o cliente parou de
// pagar e o carro foi retomado. Devolve a taxa MENSAL (recuperações ÷ contrato-meses de exposição),
// que é a base para sugerir o slider de delinquency do UE em vez de um número chutado.
function buildChurn(clientesRows, refDate) {
  const hoje = refDate || new Date();
  let dias = 0, recuperacoes = 0, encerrados = 0, ativos = 0;
  for (const r of clientesRows || []) {
    const ini = parseData(cell(r, 10));       // col K = Data Início Vínculo
    if (!ini || isNaN(ini) || ini > hoje) continue;
    const inativo = cell(r, 12).toLowerCase() === 'inativo'; // col M = Status Vínculo
    const fim = inativo ? parseData(cell(r, 11)) : null;     // col L = Data Fim Vínculo
    const ate = (fim && !isNaN(fim) && fim > ini && fim < hoje) ? fim : hoje;
    dias += (ate - ini) / 86400000;
    if (inativo) encerrados++; else ativos++;
    if (inativo && /recupera/i.test(cell(r, 13))) recuperacoes++; // col N = Motivo Fim Vínculo
  }
  const contratoMeses = dias / ((52 / 12) * 7);
  if (contratoMeses < 1) return null;
  const taxaMensal = recuperacoes / contratoMeses;
  return {
    recuperacoes, encerrados, ativos,
    contratoMeses: Math.round(contratoMeses * 10) / 10,
    taxaMensal: Math.round(taxaMensal * 10000) / 10000,             // fração (0,049 = 4,9% a.m.)
    taxaAnual: Math.round((1 - Math.pow(1 - taxaMensal, 12)) * 10000) / 10000,
  };
}
...
function build(ueSheets, importRows, clientesRows, refDate) {
  ...
  return {
    periods,
    fleets: buildFleets(importRows || []),
    losses: buildLosses(clientesRows || []),
    contratos: buildContratos(clientesRows || []),
    starts: buildStarts(clientesRows || []),
    churn: buildChurn(clientesRows || [], hoje),
    orcado,
