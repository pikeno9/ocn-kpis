...
//
// 1) o VÍNCULO encerrado por sinistro — mas ele só tem "Data Fim Vínculo" quando alguém a
//    digitou, e a planilha não data os vínculos encerrados. Medido em 03/09/2026: a única
//    perda total da frota (TYZ2D13, motivo "Sinitro - PT") vinha sem as duas datas, então
//    buildLosses devolvia {} e a máquina inteira de perda total nunca disparava para ninguém:
//    o UE projetava Subscription, Subrental e GPS de um carro que já não existe.
// 2) a DATA DO EVENTO na aba de ocorrências, onde o TIPO é "Perda Total" (TYZ2D13 → 11/05/2026).
//    É a data em que o carro parou de produzir, então ela vence a do vínculo se for anterior.
function buildLosses(clientesRows, ocorrRows) {
  const out = {};
  for (const r of clientesRows || []) {
    ...
    if (placa && d && !isNaN(d) && d.getFullYear() >= 2020) out[placa] = d.toISOString().slice(0, 10);
  }
  for (const r of ocorrRows || []) {
    if (!/perda total/i.test(cell(r, 8))) continue;   // col I = TIPO
    const placa = cell(r, 1).toUpperCase();           // col B = Placa
    const d = parseData(cell(r, 3));                  // col D = Data do Evento
    if (!placa || !d || isNaN(d) || d.getFullYear() < 2020) continue;
    const iso = d.toISOString().slice(0, 10);
    if (!out[placa] || iso < out[placa]) out[placa] = iso;
  }
  return out;
}
...
function build(ueSheets, importRows, clientesRows, refDate, ocorrRows) {
  ...
  return {
    periods,
    fleets,
    losses: buildLosses(clientesRows || [], ocorrRows || []),
