...
function build(importRows, frota, refDate, losses) {
  const hoje = refDate || new Date();
  const placas = (frota && frota.placas) || {};
  const perdas = losses || {}; // placa -> data da perda total (Sinitro - PT) — excluída da análise
  const out = [];
  for (let i = 9; i < (importRows || []).length; i++) {
    ...
    if (perdas[placa]) continue; // total loss: carro fora de circulação, não entra na utilização
    const dt = parseData(cell(importRows[i], 3));
    if (!dt) continue;
    const fr = placas[placa];
    if (!fr || fr.odo <= 0) continue; // precisa existir na frota com odômetro > 0 (inclui odômetro não sincronizado; perda total já foi excluída acima)
