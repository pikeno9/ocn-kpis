...
(function () {
  ...
  // tooltips do site inteiro: cartão claro com borda e cantos generosos, título forte e
  // marcadores redondos — a caixinha preta padrão do Chart.js destoava do resto do visual
  Object.assign(Chart.defaults.plugins.tooltip, {
    backgroundColor: 'rgba(255,255,255,.98)',
    titleColor: '#111827', bodyColor: '#4B5563', footerColor: '#8B5CF6',
    borderColor: 'rgba(17,24,39,.10)', borderWidth: 1,
    cornerRadius: 12, padding: { top: 11, bottom: 11, left: 14, right: 14 },
    titleFont: { size: 12.5, weight: 800 }, bodyFont: { size: 11.5, weight: 600 },
  ...
  function recoveryModel() {
    ...
    // ---- (a) curva de risco por idade do CONTRATO + curva de TROCA DE MOTORISTA ----
    // O contrato também acaba por outros motivos (rescisão do motorista, troca, perda total): 12
    // casos medidos. Isso NÃO gera custo, mas devolve o carro para a rua com um motorista NOVO, que
    // volta ao M0 e corre a curva de risco de novo. Sem essa realimentação o modelo assumia que
    // quem passa do M3 fica seguro para sempre e a projeção despencava para ~15% do histórico.
    // Três desfechos com consequências diferentes: RECUPERAÇÃO (guincho + recuperação + reparo +
    // multa de rescisão), DEVOLUÇÃO antecipada (reparo + multa de rescisão — o carro volta, mas
    // volta precisando de conserto em parte dos casos) e OUTRAS saídas (troca de carro, perda
    // total): só reiniciam o ciclo, sem custo próprio aqui.
    const expo = new Array(AMAX).fill(0), evt = new Array(AMAX).fill(0);
    const evtRet = new Array(AMAX).fill(0), evtOut = new Array(AMAX).fill(0);
    const recs = [], rets = [];
    let mesesExpo = 0;
    V.forEach((v) => {
      ...
      const isRet = !!(v.fim && /rescis/i.test(v.motivo || ''));   // devolução antecipada
      for (let a = 0; a < AMAX; a++) if (idade >= a) expo[a]++;
      const a0 = Math.min(AMAX - 1, Math.floor(idade));
      if (isRec) { evt[a0]++; recs.push(v); }
      else if (isRet) { evtRet[a0]++; rets.push(v); }
      else if (v.fim) evtOut[a0]++;                    // troca de carro / perda total
    });
    // CREDIBILIDADE em vez de piso fixo: hazard = (eventos + k·p0) / (expostos + k). Idade com
    // muita exposição fala por si; idade com pouca (ou nenhuma) é puxada para a média geral p0 —
    // "zero em 58 casos" vira ~0,5%, mas "zero em 3 casos" não vira zero.
    const K = 8;
    const nOut = V.filter((v) => v.fim && !/recupera|rescis/i.test(v.motivo || '')).length;
    const p0 = mesesExpo > 0 ? recs.length / mesesExpo : 0;
    const p0ret = mesesExpo > 0 ? rets.length / mesesExpo : 0;
    const p0out = mesesExpo > 0 ? nOut / mesesExpo : 0;
    const hazard = expo.map((e, a) => (evt[a] + K * p0) / (e + K));
    const hazardRet = expo.map((e, a) => (evtRet[a] + K * p0ret) / (e + K));
    const hazardOut = expo.map((e, a) => (evtOut[a] + K * p0out) / (e + K));
    // ---- (c) custo de cada desfecho, medido nos casos reais do import_jud ----
    // Cada contrato encerrado consome UM caso do import_jud — o mais próximo da data de fim, e sem
    // repetição. Filtrar só por placa somava os dois casos de EGX9H53 e TYZ2D16 (placas com dois
    // ciclos) em cada um dos contratos, inflando o ticket médio de recuperação em ~20%.
    const medir = (lista, re) => {
      let nG = 0, sG = 0, nP2 = 0, sP2 = 0, nT = 0, sT = 0;
      const usados = new Set();
      lista.forEach((v) => {
        const alvo = new Date((v.fim || U.hoje) + 'T12:00:00');
        const dist = (c) => Math.abs((c.d ? new Date(c.d + 'T12:00:00') : alvo) - alvo);
        const cand = (JB[v.placa] || []).map((c, i) => ({ c, k: v.placa + '#' + i }))
          .filter((x) => re.test(x.c.tipo || '') && !usados.has(x.k))
          .sort((a, b) => dist(a.c) - dist(b.c));
        if (!cand.length) return;
        usados.add(cand[0].k);
        const c = cand[0].c;
        const g = (c.guincho || 0) + (c.recup || 0), rp = c.repair || 0, tm = c.term || 0;
        if (g > 0) { nG++; sG += g; }
        if (rp > 0) { nP2++; sP2 += rp; }
        if (tm > 0) { nT++; sT += tm; }
      });
      return { custoRec: nG ? sG / nG : 0, custoRep: nP2 ? sP2 / nP2 : 0, pRep: lista.length ? nP2 / lista.length : 0,
        term: nT ? sT / nT : 0, pTerm: lista.length ? nT / lista.length : 0, n: lista.length };
    };
    const mRec = medir(recs, /recupera/i);
    const mRet = medir(rets, /devolu/i);
    const costRec = mRec.custoRec;                     // R$ por recuperação (guincho + recuperação)
    const costRep = mRec.custoRep;                     // R$ por reparo pós-recuperação
    const pRep = mRec.pRep;                            // ~78% das recuperações geram reparo
    // devolução antecipada: o carro volta, e em parte dos casos volta precisando de conserto
    const costRepRet = mRet.custoRep, pRepRet = mRet.pRep;
    // multa de rescisão por encerramento antecipado. Ticket médio × incidência: nem todo caso é
    // cobrado (nas devoluções só 2/3 geraram multa), então o valor por EVENTO já vem líquido disso.
    const termRec = mRec.term * mRec.pTerm, termRet = mRet.term * mRet.pTerm;
    ...
    function serieDaPlaca(pl, nSemanas) {
      ...
      const hz = hazard.map(wk), hzRet = hazardRet.map(wk), hzOut = hazardOut.map(wk);
      const idadeSem = (a) => Math.min(AMAX - 1, Math.floor(a / (52 / 12)));
      const rec = new Array(nSemanas).fill(0), ret = new Array(nSemanas).fill(0);
      const v = ativos[pl];
      if (!v) return { rec, ret };                        // carro sem contrato ativo não gera evento
      ...
      for (let s = 0; s < nSemanas; s++) {
        ...
        const recCur = cur * hz[aCur] * r;                // recuperado: custo cheio + reinicia
        const retCur = cur * hzRet[aCur];                 // devolveu: reparo + multa + reinicia
        const outCur = cur * hzOut[aCur];                 // troca/perda total: só reinicia
        let nRec = recCur, nRet = retCur, reinicia = recCur + retCur + outCur;
        for (let a = 0; a < nw.length; a++) {
          if (!nw[a]) continue;
          const ia = idadeSem(a);
          const rr = nw[a] * hz[ia], tt = nw[a] * hzRet[ia], oo = nw[a] * hzOut[ia];
          nRec += rr; nRet += tt; reinicia += rr + tt + oo;
          nw[a] -= rr + tt + oo;
        }
        rec[s] += nRec; ret[s] += nRet;
        cur -= recCur + retCur + outCur; ageCur += 1;
    ...
    function esperadasPorSemana(plates, nSemanas) {
      ...
      plates.forEach((pl) => serieDaPlaca(pl, nSemanas).rec.forEach((q, s) => { out[s] += q; }));
      return out;
    }
    ...
    function eventosPorSemana(plates, nSemanas) {
      ...
      const ret = new Array(nSemanas).fill(0), repRet = new Array(nSemanas).fill(0);
      const term = new Array(nSemanas).fill(0);          // R$ de multa de rescisão gerada
      plates.forEach((pl) => {
        const serie = serieDaPlaca(pl, nSemanas);
        let accR = 0, limR = hashU(pl), nR = 0;
        // devolução usa o MESMO conjunto de limiares girado pela razão áurea: fica descorrelacionado
        // da recuperação sem perder a boa distribuição do hash (um salt novo dava -24% no total)
        let accT = 0, limT = (hashU(pl) + 0.6180339887498949) % 1, nT = 0;
        for (let s = 0; s < nSemanas; s++) {
          accR += serie.rec[s];
          while (accR >= limR) {
            rec[s] += 1;
            term[s] += termRec;                          // toda recuperação gera multa de rescisão
            ...
            if (hashU(pl + '#' + nR) < pRep) rep[s] += 1;
            limR += 1; nR++;
          }
          accT += serie.ret[s];
          while (accT >= limT) {
            ret[s] += 1;
            term[s] += termRet;                          // devolução antecipada também gera multa
            if (hashU(pl + '@' + nT) < pRepRet) repRet[s] += 1;
            limT += 1; nT++;
          }
        }
      });
      return { rec, rep, ret, repRet, term };
    }
    _recModel = { hazard, hazardRet, hazardOut, expo, evt, evtRet, evtOut,
      costRec, costRep, pRep, costRepRet, pRepRet, termRec, termRet, mRec, mRet,
      nRecs: recs.length, nRets: rets.length, baseRate, taxaLate, taxaOk, nLate, nOk,
      fator, ativos, esperadasPorSemana, eventosPorSemana, p0, p0ret, p0out,
  ...
  function initFinance() {
    ...
    function finActuals() {
      ...
      (U.fleets || []).forEach((f) => {
        ...
        if (IRs) {
          ...
          // sem vencimento = paga à vista na assinatura (mês de início da frota)
          IRs.parcelas.forEach((p2) => {
            const d = p2.venc ? new Date(p2.venc + 'T12:00:00') : ini;
    ...
    let costsFleetTot = false;    // Cost by fleet: false = por carro·mês, true = total do período
    let costsKmTop = false;       // Odometer: destacar os 10 maiores km/semana
    let costsKmView = 'trend';   // gráfico novo do overview: 'trend' (média da frota) | 'breaks' (quedas por placa)
    ...
    const COSTS_HELP = {
      ...
      percar: { t: 'This line’s own chart', d: 'Each cost line shows its own view here: the contracted rent by fleet, the insurance installments stacked by fleet, the odometer of every plate, the fines each client accumulates, or each recovery/repair case with its cost.' },
    ...
    const COSTS_INFO = {
      ...
      'Repair cost': 'Damages + cleaning + others per judicial case (import_jud) when a car comes back — after a repossession or after a voluntary early return, which the forecast projects on its own risk curve.',
    ...
    function costsRealByFleet() {
      ...
      (U.fleets || []).forEach((f) => {
        ...
        if (IRc) {
          ...
          // parcela sem vencimento foi paga à vista na assinatura — cai no mês de início da frota
          IRc.parcelas.forEach((p2) => { add('Insurance', moOf(p2.venc || f.inicio), p2.valor * IRc.share); });
        ...
        if (curM >= 0 && curM < FIN_MONTHS - 1) {
          ...
          joga(EVf.repRet, 'Repair cost', RMf.costRepRet);   // devolução também dá avaria
        }
        ...
        // Sem isso o share compararia custo de frota real com receita do plano de coortes.
        const R = new Array(FIN_MONTHS).fill(0);
        ...
        out.fleets.push({ id: f.id, arr: L, accr: A, cars: carsArr, rev: R });
      });
    ...
    // soma das parcelas do seguro das frotas dadas (com o rateio por frota): dentro do ano
    // exibido × depois dele — o que vence em 2027 não cabe no gráfico anual mas não pode sumir
    function insParcelasRS(fleetIds) {
      let ano = 0, depois = 0;
      (fleetIds || []).forEach((id) => {
        const r = insRealShare(id); if (!r) return;
        r.parcelas.forEach((p) => {
          const y = p.venc ? +p.venc.slice(0, 4) : finYear;   // à vista = ano de início da frota
          if (y > finYear) depois += p.valor * r.share; else ano += p.valor * r.share;
        });
      });
      return { ano, depois, total: ano + depois };
    }
    function costsAvgPerCarMonth(line, RF, fleets) {
      ...
      // Insurance com a aba de parcelas reais: a média por carro·mês é CONTRATADA — o prêmio
      // total da apólice ÷ carros ÷ 12 meses de cobertura. Dividir o CAIXA desembolsado pelos
      // carro-meses corridos inflava o número (as parcelas correm à frente da cobertura: paga-se
      // em ~6 meses o risco de 12).
      if (line === 'Insurance') {
        let premTot = 0, carsTot = 0, nF = 0;
        const seen = {};
        fleets.forEach((f) => {
          const r = insRealShare(f.id);
          if (!r) return;
          const meta = ((OCN.ue || {}).fleets || []).find((x) => x.id === f.id);
          premTot += r.total; carsTot += meta ? (meta.cars || (meta.placas || []).length || 0) : 0;
          if (!seen[r.host]) { seen[r.host] = 1; nF++; }
        });
        if (premTot > 0 && carsTot > 0) {
          const fx = finPar('__fin_fx__') || 5.5;
          return { v: (premTot / carsTot / 12) / fx, proj: false, cash: false, param: true, insContract: true, nFleets: nF, cars: carsTot, cm: null, tot: premTot / fx };
        }
      }
    ...
    function renderCostsMain(P, H) {
      ...
      document.getElementById('ccDrillT').innerHTML = `<span class="costs-pk-dot" style="background:${DC}"></span>${escH(COSTS_LABEL(drillL))} by fleet`;
      const nM = (H.mSel === 'ytd' && RF.curM >= 0) ? RF.curM + 1 : FIN_MONTHS;   // YTD encerra hoje
      const cutM = (a) => a.slice(0, nM);
      const MONL = Array.from({ length: nM }, (_, m) => monthLbl(m));
      mk('ccDrill', { type: 'bar', data: { labels: MONL, datasets: rfF.map((f) => (
          { label: 'Fleet ' + f.id, fleetId: f.id, data: cutM((f.arr[drillL] || []).map((v, m) => (RF.curM >= 0 && m <= RF.curM ? Math.round(v * K) : null))), backgroundColor: FLEET_COLOR(f.id), stack: 's', maxBarThickness: 40, borderRadius: 2 }
        )).concat(showProj ? [{ label: 'Projected', proj: true, data: cutM(projArr), backgroundColor: costsStripe(DC), borderColor: costsTint(DC, .55), borderWidth: 1, stack: 's', maxBarThickness: 40, borderRadius: 2 }] : []) },
      ...
      costsAvgHelp = { t: COSTS_LABEL(drillL) + ' — avg per car · month', d: AV.insContract
        ? 'The real policies from the "insurance" sheet tab: total premium of every policy (' + money(AV.tot) + ' across ' + AV.nFleets + ' policies covering ' + ccNum(AV.cars) + ' cars) divided by the cars and by the 12 months each policy covers. This is the CONTRACTED monthly cost of insuring one car. Dividing the cash disbursed so far by the car-months run so far would overstate it, because the installments are paid over ~6 months while the coverage runs 12 — you pay ahead of the risk.' + baseTxt
        : AV.param
      ...
      const shareLabels = Array.from({ length: nM }, (_, m) => monthLbl(m)).concat([String(finYear)]);
      const totCol = cutM(shareTotM).concat([anoTot]);
      const pctOf = (L) => { const a = H.lineOfL(L); const yr = a.reduce((s, v) => s + (v || 0), 0);
        return cutM(a.map((v, m) => (shareTotM[m] > 0 ? (v / shareTotM[m]) * 100 : null))).concat([anoTot > 0 ? (yr / anoTot) * 100 : null]); };
      const othersPct = new Array(nM + 1).fill(0);
      COSTS_LIST.filter((L) => !shareTop.includes(L)).forEach((L) => { const p = pctOf(L); for (let m = 0; m <= nM; m++) othersPct[m] += p[m] || 0; });
      const futuro = (m) => m < nM && RF.curM >= 0 && m > RF.curM;   // projeção mais clara
      const isAno = (m) => m === nM;
      ...
      mk('ccShare', { type: 'bar', data: { labels: shareLabels, datasets: shareTop.map((L) => (
        ...
        options: { responsive: true, maintainAspectRatio: false,
          ...
          scales: { x: { stacked: true, grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5 }, color: (c) => (c.index === nM ? '#111827' : '#6B7280') } },
            y: { stacked: true, min: 0, max: 100, grid: { display: false }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: (v) => v + '%' } } } } });
    }
    // =========== gráfico PRÓPRIO de cada linha (caixa esquerda do grid pequeno) ===========
    // Cada linha de custo responde a uma pergunta diferente — um mensal por carro genérico não
    ...
    // ---- base de RECUPERAÇÕES: vínculos encerrados por recuperação × casos do jud ----
    // A import_jud também traz DEVOLUÇÕES e perda total (col F, "Tipo do evento") — aqui só o que
    // é recuperação de verdade. Cada evento junta a data/duração do vínculo (import_clientes) com
    // os custos de guincho e de recuperação do caso judicial da placa.
    function recoveryData() {
      const U = OCN.ue || {};
      const fx = finPar('__fin_fx__') || 5.5;
      const caseOf = {}; const looseCases = [];   // casos jud de recuperação, por placa
      Object.entries(((U.judBase || {}).placas) || {}).forEach(([pl, a]) => a.forEach((c) => {
        if (!/recupera/i.test(c.tipo || '')) return;
        const o = caseOf[pl] = caseOf[pl] || { guincho: 0, recup: 0, d: null };
        o.guincho += c.guincho || 0; o.recup += c.recup || 0;
        if (c.d && (!o.d || c.d < o.d)) o.d = c.d;
    ...
    // Implementações compactas e testadas dos testes clássicos — o site não tem Python, então
    // regressão logística (IRLS), Mann-Whitney, qui-quadrado, Kaplan-Meier e Cox vivem aqui.
    const erf = (x) => { const s = x < 0 ? -1 : 1; x = Math.abs(x);
      ...
      return s * y; };
    const normCdf = (z) => .5 * (1 + erf(z / Math.SQRT2));
    const pWald = (z) => 2 * (1 - normCdf(Math.abs(z)));
    const gammaln = (x) => { const g = [676.5203681218851, -1259.1392167224028, 771.32342877765313, -176.61502916214059, 12.507343278686905, -.13857109526572012, 9.9843695780195716e-6, 1.5056327351493116e-7];
      if (x < .5) return Math.log(Math.PI / Math.sin(Math.PI * x)) - gammaln(1 - x);
      x -= 1; let a = .99999999999980993; const t = x + 7.5;
      for (let i = 0; i < 8; i++) a += g[i] / (x + i + 1);
      return .5 * Math.log(2 * Math.PI) + (x + .5) * Math.log(t) - t + Math.log(a); };
    // Q(a,x) — gama incompleta regularizada superior (série + fração continuada, NR)
    function gammaQ(a, x) {
      if (x < 0 || a <= 0) return NaN;
      if (x === 0) return 1;
      if (x < a + 1) {   // série para P, devolve 1−P
      ...
      let b = x + 1 - a, c = 1e30, d = 1 / b, h = d;   // fração continuada para Q
      for (let i = 1; i < 200; i++) {
        const an = -i * (i - a); b += 2;
    ...
    // regressão LOGÍSTICA por IRLS (covariáveis padronizadas por dentro; betas devolvidos na escala original)
    function logisticFit(rows, keys) {
      const n = rows.length, p = keys.length + 1;
      if (n < p + 5) return null;
      const mu = keys.map((k) => rows.reduce((s, r) => s + r[k], 0) / n);
      const sd = keys.map((k, j) => Math.sqrt(rows.reduce((s, r) => s + (r[k] - mu[j]) * (r[k] - mu[j]), 0) / n) || 1);
      const X = rows.map((r) => [1, ...keys.map((k, j) => (r[k] - mu[j]) / sd[j])]);
      const y = rows.map((r) => (r.y ? 1 : 0));
      let beta = new Array(p).fill(0);
      const solve = (A, b) => {   // Gauss com pivoteamento parcial
        const M = A.map((row, i) => [...row, b[i]]);
        for (let c = 0; c < p; c++) {
          let piv = c; for (let r = c + 1; r < p; r++) if (Math.abs(M[r][c]) > Math.abs(M[piv][c])) piv = r;
          if (Math.abs(M[piv][c]) < 1e-12) return null;
          [M[c], M[piv]] = [M[piv], M[c]];
          for (let r = 0; r < p; r++) { if (r === c) continue; const f = M[r][c] / M[c][c]; for (let k2 = c; k2 <= p; k2++) M[r][k2] -= f * M[c][k2]; }
        }
        return M.map((row, i) => row[p] / row[i]);
      };
      let H = null;
      for (let it = 0; it < 60; it++) {
        const eta = X.map((xi) => xi.reduce((s, v, j) => s + v * beta[j], 0));
        const pr = eta.map((e) => 1 / (1 + Math.exp(-Math.max(-30, Math.min(30, e)))));
        const grad = new Array(p).fill(0);
        H = Array.from({ length: p }, () => new Array(p).fill(0));
        for (let i = 0; i < n; i++) {
          const w = Math.max(1e-9, pr[i] * (1 - pr[i])), d = y[i] - pr[i];
          for (let j = 0; j < p; j++) {
            grad[j] += X[i][j] * d;
            for (let k2 = 0; k2 <= j; k2++) H[j][k2] += w * X[i][j] * X[i][k2];
          }
        }
        for (let j = 0; j < p; j++) { H[j][j] += 1e-6; for (let k2 = j + 1; k2 < p; k2++) H[j][k2] = H[k2][j]; }
        const step = solve(H.map((r) => [...r]), grad);
        if (!step) return null;
        beta = beta.map((b, j) => b + step[j]);
        if (Math.max(...step.map(Math.abs)) < 1e-8) break;
      }
      // erro-padrão = sqrt(diag(H⁻¹)) — inverte resolvendo p sistemas
      const se = [];
      for (let j = 0; j < p; j++) { const e = new Array(p).fill(0); e[j] = 1; const col = solve(H.map((r) => [...r]), e); se.push(col ? Math.sqrt(Math.max(0, col[j])) : NaN); }
      // volta à escala ORIGINAL das covariáveis
      const out = keys.map((k, j) => ({ k, beta: beta[j + 1] / sd[j], se: se[j + 1] / sd[j] }));
      out.forEach((o) => { o.z = o.beta / o.se; o.p = pWald(o.z); o.or = Math.exp(o.beta); });
      return { coefs: out, n, events: y.reduce((a, b) => a + b, 0) };
    }
    // Mann-Whitney U (aproximação normal com correção de empates), bicaudal
    function mannWhitney(a, b) {
      const all = a.map((v) => ({ v, g: 0 })).concat(b.map((v) => ({ v, g: 1 }))).sort((x, y) => x.v - y.v);
      const n = all.length, n1 = a.length, n2 = b.length;
      if (!n1 || !n2) return null;
      let i = 0; const tieT = [];
      while (i < n) { let j = i; while (j + 1 < n && all[j + 1].v === all[i].v) j++;
        const rank = (i + j + 2) / 2; for (let k2 = i; k2 <= j; k2++) all[k2].rank = rank;
        if (j > i) tieT.push(j - i + 1); i = j + 1; }
      const R1 = all.filter((x) => x.g === 0).reduce((s, x) => s + x.rank, 0);
      const U1 = R1 - n1 * (n1 + 1) / 2, U = Math.min(U1, n1 * n2 - U1);
      const tieAdj = tieT.reduce((s, t) => s + (t * t * t - t), 0);
      const sd = Math.sqrt(n1 * n2 / 12 * ((n + 1) - tieAdj / (n * (n - 1))));
      const z = sd > 0 ? (U - n1 * n2 / 2 + .5) / sd : 0;
      return { U, z, p: pWald(z) };
    }
    // Kaplan-Meier: S(t) + vida média restrita (RMST) até tMax
    function kmSurvival(rows, tMax) {   // rows: {t, ev}
      const evTimes = [...new Set(rows.filter((r) => r.ev).map((r) => r.t))].sort((a, b) => a - b);
      let s = 1; const curve = [{ t: 0, s: 1 }];
      evTimes.forEach((t) => {
        const nRisk = rows.filter((r) => r.t >= t).length;
        const d = rows.filter((r) => r.ev && r.t === t).length;
        if (nRisk > 0) { s *= 1 - d / nRisk; curve.push({ t, s }); }
      });
      let rmst = 0, prevT = 0, prevS = 1;   // integral de S(t) até tMax (escada)
      curve.slice(1).forEach((pt) => { const t = Math.min(pt.t, tMax); if (t > prevT) rmst += prevS * (t - prevT); prevT = t; prevS = pt.s; });
      if (prevT < tMax) rmst += prevS * (tMax - prevT);
      return { curve, rmst };
    }
    // Cox proporcional (parcial de Breslow, Newton) — covariáveis padronizadas por dentro
    function coxFit(rows, keys) {   // rows: {t, ev, <keys>}
      const n = rows.length, p = keys.length;
      const events = rows.filter((r) => r.ev).length;
      if (events < p + 3) return null;
      const mu = keys.map((k) => rows.reduce((s, r) => s + r[k], 0) / n);
      const sd = keys.map((k, j) => Math.sqrt(rows.reduce((s, r) => s + (r[k] - mu[j]) * (r[k] - mu[j]), 0) / n) || 1);
      const X = rows.map((r) => keys.map((k, j) => (r[k] - mu[j]) / sd[j]));
      let beta = new Array(p).fill(0);
      const solve = (A, b) => { const M = A.map((row, i) => [...row, b[i]]);
      ...
      for (let it = 0; it < 60; it++) {
        const grad = new Array(p).fill(0);
        H = Array.from({ length: p }, () => new Array(p).fill(0));
        const evTimes = [...new Set(rows.filter((r) => r.ev).map((r) => r.t))];
        for (const t of evTimes) {
          const risk = []; rows.forEach((r, i) => { if (r.t >= t) risk.push(i); });
          const dIdx = []; rows.forEach((r, i) => { if (r.ev && r.t === t) dIdx.push(i); });
          let s0 = 0; const s1 = new Array(p).fill(0), s2 = Array.from({ length: p }, () => new Array(p).fill(0));
          for (const i of risk) {
            const w = Math.exp(Math.max(-30, Math.min(30, X[i].reduce((s, v, j) => s + v * beta[j], 0))));
            s0 += w;
            for (let j = 0; j < p; j++) { s1[j] += w * X[i][j]; for (let k2 = 0; k2 <= j; k2++) s2[j][k2] += w * X[i][j] * X[i][k2]; }
          }
          const d = dIdx.length;
          for (let j = 0; j < p; j++) {
            grad[j] += dIdx.reduce((s, i) => s + X[i][j], 0) - d * s1[j] / s0;
            for (let k2 = 0; k2 <= j; k2++) H[j][k2] += d * (s2[j][k2] / s0 - (s1[j] / s0) * (s1[k2] / s0));
          }
        }
        for (let j = 0; j < p; j++) { H[j][j] += 1e-6; for (let k2 = j + 1; k2 < p; k2++) H[j][k2] = H[k2][j]; }
        const step = solve(H.map((r) => [...r]), grad);
      ...
      const se = [];
      for (let j = 0; j < p; j++) { const e = new Array(p).fill(0); e[j] = 1; const col = solve(H.map((r) => [...r]), e); se.push(col ? Math.sqrt(Math.max(0, col[j])) : NaN); }
      const coefs = keys.map((k, j) => { const b = beta[j] / sd[j], s = se[j] / sd[j];
        return { k, beta: b, se: s, z: b / s, p: pWald(b / s), hr: Math.exp(b) }; });
      return { coefs, n, events };
    }
    // ---- dataset por CONTRATO para as análises (vínculo × multas × km × pagamentos × modelo) ----
    function recAnalysisData() {
      const U = OCN.ue || {};
      const fx = finPar('__fin_fx__') || 5.5;
      const hojeIso = U.hoje || new Date().toISOString().slice(0, 10);
      const modelOf = {}; (U.fleets || []).forEach((f) => (f.placas || []).forEach((pl) => { modelOf[pl] = f.modelLabel || f.model; }));
      const MB = ((U.multasBase || {}).placas) || {};
      const PAY = ((U.pagamentos || {}).placas) || {};
      const UT = {}; (((OCN.utilization) || {}).plates || []).forEach((r) => { UT[r.plate] = r.kmWeek; });
      const rows = [];
      (U.vinculos || []).forEach((v) => {
        const end = v.fim || hojeIso;
        const durM = Math.max(.25, (new Date(end + 'T12:00:00') - new Date(v.ini + 'T12:00:00')) / 86400000 / 30.44);
        const rec = !!(v.fim && /recupera/i.test(v.motivo || ''));
        const troca = !!(v.fim && /troca/i.test(v.motivo || ''));
        let fines = 0, finesV = 0;
        (MB[v.placa] || []).forEach((x) => { const d = x.inf || x.email || x.venc; if (d && d >= v.ini && d <= end) { fines++; finesV += x.v || 0; } });
        // 2ª semana do contrato (a 1ª é paga na retirada): atrasou ou nem pagou?
        // A matriz só traz semanas PAGAS, com a flag `a` = pagamento em atraso. Então:
        // atrasou = 2ª cobrança paga com atraso (a=1) OU ausente da matriz com o contrato
        // já na 3ª semana de vida (nunca pagou). Contrato mais novo que isso fica de fora.
        const weeks = (PAY[v.placa] || []).filter((s) => s.v && s.v >= v.ini && s.v <= end).sort((a, b) => (a.v < b.v ? -1 : 1));
        const durW = (new Date(end + 'T12:00:00') - new Date(v.ini + 'T12:00:00')) / 86400000 / 7;
        let delay2 = null;
        if (durW >= 3) { const w2 = weeks[1]; delay2 = !w2 ? true : (w2.a === 1); }
        rows.push({ placa: v.placa, nome: v.nome, ini: v.ini, fim: v.fim, durM, rec, troca,
          ended: !!(v.fim && !troca),                       // devolução/recuperação/perda encerram o contrato
          finesPM: fines / durM, finesVPM: finesV / fx / durM,
          kmWeek: UT[v.placa] != null ? UT[v.placa] : null,
          model: modelOf[v.placa] || '?', delay2 });
      });
      return rows;
    }
    ...
    function renderCostsLeft(C, H) {
      ...
      // rótulo em PÍLULA com fundo e alternância cima/baixo — texto solto sobre os pontos virava
      // sopa ilegível quando dois outliers caíam perto
      const DL_BG = () => (document.documentElement.classList.contains('dark') ? 'rgba(21,22,27,.92)' : 'rgba(255,255,255,.92)');
      const outDL = (lblOf) => ({ display: isOut, align: (c2) => (c2.dataIndex % 2 ? 'top' : 'bottom'), offset: 6,
        color: '#B91C1C', backgroundColor: DL_BG, borderColor: 'rgba(185,28,28,.4)', borderWidth: 0.5, borderRadius: 5,
        padding: { top: 2, bottom: 1, left: 5, right: 5 },
        font: { size: 9.5, weight: 700 }, formatter: (v, c2) => { const p = ptOf(c2); return p ? lblOf(p) : ''; }, clamp: true });
      ...
      // o painel de análises do Recovery é SEMPRE largura cheia (os gráficos precisam do espaço);
      // o "Repossessions per month" desce em largura cheia logo abaixo
      const alwaysBig = costsSel === 'Recovery cost';
      if (expBtn) {
      ...
      const hojeIso = (OCN.ue && OCN.ue.hoje) || new Date().toISOString().slice(0, 10);
      const MS = 86400000;
      const monthFrac = (iso) => (new Date(iso + 'T12:00:00') - new Date(finYear, 0, 1, 12)) / MS / 30.44;
      const MON_TICK = { min: 0, max: 12, grid: { display: false }, border: { display: false },
        ticks: { font: CC_FONT, color: '#6B7280', stepSize: 1, callback: (v) => (v >= 0 && v < 12 ? monthLbl(v) : '') } };
      costsLeftHelp = null;
      if (costsSel === 'Subrental fee') {
        ...
        // tons SUAVES por modelo, só aqui: o arco-íris saturado gritava num gráfico que compara
        // alturas parecidas — mesma família de cor, croma bem menor
        const SUBR_COLOR = { Polo: '#8B80C9', Argo: '#C9B074', Tera: '#7FA8C9' };
        // botões-filtro por MODELO (também funcionam de legenda das cores)
        const models = [...new Set(all.map((r) => r.f.model))];
        if (ctl) {
          ctl.innerHTML = `<button type="button" class="ccl-btn${costsSubrModel == null ? ' on' : ''}" data-m="">All</button>` +
            models.map((m) => { const meta = all.find((r) => r.f.model === m).f;
              return `<button type="button" class="ccl-btn${costsSubrModel === m ? ' on' : ''}" data-m="${escH(m)}" style="--c:${SUBR_COLOR[m] || C}"><span class="dot"></span>${escH(meta.modelLabel || m)}</button>`; }).join('');
        ...
        mk('ccPerCar', { type: 'bar', data: { labels: rows.map((r) => 'Fleet ' + r.f.id),
            datasets: [{ data: rows.map((r) => Math.round((r.v / fx) * K)), backgroundColor: rows.map((r) => SUBR_COLOR[r.f.model] || C), borderRadius: 6, maxBarThickness: 58 }] },
          options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 22 } },
            plugins: { legend: { display: false },
              datalabels: { anchor: 'end', align: 'top', offset: 1, color: '#374151', font: { size: 11, weight: 800 }, formatter: ccNum },
              tooltip: { displayColors: false, callbacks: {
                title: (items) => 'Fleet ' + rows[items[0].dataIndex].f.id + ' · ' + (rows[items[0].dataIndex].f.modelLabel || rows[items[0].dataIndex].f.model),
                label: (c) => cs + ' ' + ccNum(c.parsed.y) + ' per car · month',
                footer: (items) => (rows[items[0].dataIndex].f.cars || 0) + ' cars in this fleet' } } },
            scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 11, weight: 700 }, color: '#374151', maxRotation: 0, minRotation: 0 } },
              y: { grid: { display: false }, border: { display: false }, beginAtZero: true, grace: '10%', ticks: { font: CC_FONT, color: '#6B7280', callback: ccK } } } } });
        return;
      }
      if (costsSel === 'Insurance') {
        const cols = [];
        fleetsMeta.forEach((f) => {
          const r = insRealShare(f.id); if (!r || !(r.share > 0)) return;
          const carsCov = Math.round((f.cars || (f.placas || []).length || 0) / r.share);   // carros cobertos pela apólice (frota 4+5 = 39)
        ...
        mk('ccPerCar', { type: 'bar', data: { labels: cols.map((c2) => 'Fleet ' + c2.f.id), datasets: dsets },
          plugins: [{ id: 'insTot', afterDatasetsDraw(ch) {
            const { ctx } = ch; const tots = {}, top = {};
            ch.data.datasets.forEach((ds, di) => { const meta = ch.getDatasetMeta(di);
              meta.data.forEach((el, i2) => { const v = ds.data[i2]; if (v == null) return; tots[i2] = (tots[i2] || 0) + v; top[i2] = Math.min(top[i2] == null ? Infinity : top[i2], el.y); }); });
            const fam = getComputedStyle(document.body).fontFamily;
            ctx.save(); ctx.textAlign = 'center';
            // total por carro acima da pilha + MODELO discreto abaixo do "Fleet N"
            ctx.font = '800 10.5px ' + fam; ctx.fillStyle = '#374151';
            Object.keys(tots).forEach((i2) => { if (tots[i2] > 0) ctx.fillText(ccNum(tots[i2]), ch.getDatasetMeta(0).data[i2].x, top[i2] - 5); });
            ctx.font = '600 8.5px ' + fam; ctx.fillStyle = '#9CA3AF';
            cols.forEach((c2, i2) => { const el = ch.getDatasetMeta(0).data[i2]; if (el) ctx.fillText(c2.f.modelLabel || c2.f.model, el.x, ch.chartArea.bottom + 30); });
            ctx.restore();
          } }],
          options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 22, bottom: 16 } },
            plugins: { legend: { display: false },
              tooltip: { displayColors: false, callbacks: {
                title: (items) => 'Fleet ' + cols[items[0].dataIndex].f.id + ' · ' + (cols[items[0].dataIndex].f.modelLabel || cols[items[0].dataIndex].f.model),
                label: (c2) => {
                  const pc = cols[c2.dataIndex].parcelas[c2.datasetIndex];
                  return `Installment ${c2.datasetIndex + 1}: ${cs} ${ccNum(c2.parsed.y)} per car`;
                },
                footer: (items) => { const pc = cols[items[0].dataIndex].parcelas[items[0].datasetIndex]; return pc && pc.venc ? 'Due ' + pc.venc : 'Paid upfront at signing'; } } } },
            scales: { x: { stacked: true, grid: { display: false }, border: { display: false }, ticks: { font: { size: 11, weight: 700 }, color: '#374151', maxRotation: 0, minRotation: 0 } },
              y: { stacked: true, grid: { display: false }, border: { display: false }, beginAtZero: true, grace: '12%', ticks: { font: CC_FONT, color: '#6B7280', callback: ccK } } } } });
        return;
      }
      if (costsSel === 'Maintenance') {
        const UT = ((OCN.utilization || {}).plates) || [];
        if (!UT.length) { show(false); return; }
        show(true); title('Odometer by car age');
        costsLeftHelp = { t: 'Odometer × car age', d: 'Every plate from the fleet-site API: how many km the car has run (vertical) against how old it is (horizontal, months since delivery). The dashed line is the average pace of the whole fleet in km/week — dots above it are cars running harder than average, and each horizontal gridline is one 10.000 km revision milestone, so you can see how close each car is to its next service.' };
        const pts = UT.map((r) => ({ x: r.daysElapsed / 30.44, y: r.odo, plate: r.plate, kmWeek: r.kmWeek, m: r.modelLabel, driver: r.driver || null }));
        const xMax = Math.max(4, Math.ceil(Math.max(...pts.map((p) => p.x)) + .5));
        ...
        // só os 8 MAIS FORTES ganham rótulo — todos além de 2σ mantêm o anel, mas 20 etiquetas
        // simultâneas eram exatamente a sopa ilegível do print
        const rank = pts.map((p, i2) => ({ p, d: Math.abs(resid[i2]) })).filter((x) => x.d > 2 * rSd).sort((a, b) => b.d - a.d);
        rank.forEach((x, k) => { x.p.out = true; x.p.lbl = k < 8; });
        outlierBtn();
        // top-10 de km/semana: quem está ACELERANDO a frota — âmbar, para não confundir com os
        // outliers (vermelho, distância da linha média)
        const hotRank = [...pts].sort((a, b) => (b.kmWeek || 0) - (a.kmWeek || 0)).slice(0, 10);
        hotRank.forEach((p, k) => { p.hot = true; p.hotLbl = k < 10; });
        if (ctl) {
          const bh = document.createElement('button');
          bh.type = 'button'; bh.className = 'ccl-btn' + (costsKmTop ? ' on' : ''); bh.style.setProperty('--c', '#D97706');
          bh.innerHTML = '<span class="dot"></span>km/wk';
          bh.addEventListener('click', () => { costsKmTop = !costsKmTop; renderCosts(); });
          ctl.appendChild(bh);
        }
        const isHot = (c2) => { const p = ptOf(c2); return !!(costsKmTop && p && p.hot); };
        const odoR = (c2) => (isOut(c2) || isHot(c2) ? 6.5 : 4);
        const odoBorder = (c2) => (isOut(c2) ? '#DC2626' : (isHot(c2) ? '#D97706' : '#fff'));
        const odoBorderW = (c2) => (isOut(c2) || isHot(c2) ? 2 : 1);
        const odoDL = { display: (c2) => { const p = ptOf(c2); return !!(p && ((costsOutliers && p.out && p.lbl) || (costsKmTop && p.hot))); },
          align: (c2) => (c2.dataIndex % 2 ? 'top' : 'bottom'), offset: 6, clamp: true,
          color: (c2) => (isOut(c2) ? '#B91C1C' : '#B45309'), backgroundColor: DL_BG,
          borderColor: (c2) => (isOut(c2) ? 'rgba(185,28,28,.4)' : 'rgba(180,83,9,.4)'), borderWidth: 0.5, borderRadius: 5,
          padding: { top: 2, bottom: 1, left: 5, right: 5 }, font: { size: 9.5, weight: 700 },
          formatter: (v, c2) => { const p = ptOf(c2); return p ? (isHot(c2) && !isOut(c2) ? p.plate + ' · ' + Math.round(p.kmWeek) : p.plate) : ''; } };
        ...
        mk('ccPerCar', { type: 'scatter', data: { datasets: bands.map((bd) => (
            { label: bandLbl(bd.b) + ' km', data: bd.pts, backgroundColor: bd.col,
              pointRadius: odoR, pointHoverRadius: 7, pointBorderColor: odoBorder, pointBorderWidth: odoBorderW,
              datalabels: odoDL }
          )).concat([
          ...
          // o valor da média é desenhado DEPOIS dos pontos — antes as bolinhas cobriam o número
          afterDatasetsDraw(ch) {
            const { ctx, chartArea: a } = ch, ys = ch.scales.y;
            const fam = getComputedStyle(document.body).fontFamily;
            ctx.save();
            const yEnd = ys.getPixelForValue(Math.min(kmWk * (xMax * 30.44 / 7), ys.max));
            ctx.font = '800 10.5px ' + fam; ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
            const txt = ccNum(kmWk) + ' km/week';
            const tw = ctx.measureText(txt).width + 12, yC = Math.max(a.top + 12, yEnd - 14);
            ctx.fillStyle = 'rgba(255,255,255,.94)';
            ctx.strokeStyle = 'rgba(75,85,99,.35)'; ctx.lineWidth = 1;
            ctx.beginPath(); ctx.roundRect(a.right - tw - 4, yC - 9, tw, 18, 9); ctx.fill(); ctx.stroke();
            ctx.fillStyle = '#374151';
            ctx.fillText(txt, a.right - 10, yC + 0.5);
            ctx.restore();
          } }],
          options: { responsive: true, maintainAspectRatio: false,
            plugins: { legend: { labels: CC_LEG },
              tooltip: { displayColors: false, callbacks: { label: (c2) => { const p = c2.raw;
                return p && p.plate ? [`${p.plate} · ${ccNum(p.y)} km · M${p.x.toFixed(1)} · ${ccNum(p.kmWeek)} km/wk`].concat(p.driver ? [p.driver] : []) : c2.dataset.label; } } } },
      ...
      if (costsSel === 'Traffic fines (out)') {
        const vinc = ((OCN.ue || {}).vinculos) || [];
        const MB = (((OCN.ue || {}).multasBase) || {}).placas || {};
        if (!vinc.length) { show(false); return; }
        show(true); title('Fines per client');
        // toggle: valor total das multas × NÚMERO de multas
        if (ctl) {
          ctl.innerHTML = `<button type="button" class="ccl-btn${!costsFinesCount ? ' on' : ''}" data-m="v" style="--c:${C}">Total value</button>` +
            `<button type="button" class="ccl-btn${costsFinesCount ? ' on' : ''}" data-m="n" style="--c:${C}">Number of fines</button>`;
          ctl.querySelectorAll('.ccl-btn').forEach((b) => b.addEventListener('click', () => { costsFinesCount = b.dataset.m === 'n'; renderCosts(); }));
        }
        costsLeftHelp = { t: 'Fines per client', d: 'One dot per CLIENT (each vínculo of the clients base): the traffic fines accumulated on their plate (vertical — total value we pay LM, or the count of fines, per the toggle) against how long they have held the contract (horizontal, months from start to today — or to the end, for terminated contracts, shown in grey). Dots high and to the left are clients piling up fines fast.' };
        const mkPts = (list) => list.map((v) => {
          const end = v.fim || hojeIso;
          const mo = Math.max(0, (new Date(end + 'T12:00:00') - new Date(v.ini + 'T12:00:00')) / MS / 30.44);
          let tot = 0, n = 0;
          (MB[v.placa] || []).forEach((x) => { const d = x.inf || x.email || x.venc; if (d && d >= v.ini && d <= end) { tot += x.v || 0; n++; } });
        ...
        const rankF = allP.filter((p) => p.y > meanY + 2 * sdY).sort((a, b) => b.y - a.y);
        rankF.forEach((p, k) => { p.out = true; p.lbl = k < 8; });   // anel em todos, rótulo só nos 8 piores
        outlierBtn();
        ...
        finesDL.display = (c2) => { const p = ptOf(c2); return !!(costsOutliers && p && p.out && p.lbl); };
        mk('ccPerCar', { type: 'scatter', data: { datasets: [
          ...
          options: { responsive: true, maintainAspectRatio: false,
            ...
            scales: { x: { min: 0, grid: { display: false }, border: { display: false },
                title: { display: true, text: 'months of contract', color: '#9CA3AF', font: { size: 10 } },
                ticks: { font: CC_FONT, color: '#6B7280', stepSize: 1 } },
              y: { beginAtZero: true, grace: '8%', border: { display: false }, grid: { color: 'rgba(120,120,140,.10)' },
                ticks: { font: CC_FONT, color: '#6B7280', callback: costsFinesCount ? undefined : ccK, precision: 0 } } } } });
        return;
      }
      if (costsSel === 'Recovery cost') {
        // ============ O QUE CAUSA AS RECUPERAÇÕES — painel de análises ============
        // Seis análises, cada uma no seu botão: regressão logística (risco por multas/mês, km e
        // idade), boxplot + Mann-Whitney do km/semana, tabela por modelo + qui-quadrado,
        // sobrevivência (Kaplan-Meier + Cox), atraso da 2ª semana e queda brusca de uso.
        const AR = recAnalysisData();
        if (!AR.length) { show(false); return; }
        show(true); title('What drives repossessions');
        const statsEl = document.getElementById('cclStats');
        if (statsEl) { statsEl.style.display = ''; statsEl.innerHTML = ''; }
        if (ctl) {
          const TB = [['logit', 'Risk model'], ['km', 'km/week'], ['model', 'By model'], ['surv', 'Survival'], ['pay2', '2nd week'], ['drop', 'Usage drop']];
          ctl.innerHTML = TB.map(([k, lb]) => `<button type="button" class="ccl-btn${costsCorrTab === k ? ' on' : ''}" data-m="${k}" style="--c:${C}">${lb}</button>`).join('');
          ctl.querySelectorAll('.ccl-btn').forEach((b) => b.addEventListener('click', () => { costsCorrTab = b.dataset.m; renderCosts(); }));
        }
        const fmtP = (p) => (p == null || isNaN(p) ? '—' : (p < .001 ? '<0,001' : p.toFixed(3).replace('.', ',')));
        const sigCls = (p) => (p != null && p < .05 ? 'sig' : 'nsig');
        const read = (html) => `<div class="ccl-read">${html}</div>`;
        const quart = (arr) => { const a = [...arr].sort((x, y) => x - y); const q = (f) => { const i = (a.length - 1) * f; const lo = Math.floor(i); return a[lo] + (a[Math.min(a.length - 1, lo + 1)] - a[lo]) * (i - lo); }; return { p5: q(.05), q1: q(.25), med: q(.5), q3: q(.75), p95: q(.95) }; };
        // ---------- 1) REGRESSÃO LOGÍSTICA: recuperado ~ multas/mês + km/semana + idade ----------
        if (costsCorrTab === 'logit') {
          costsLeftHelp = { t: 'Risk model — logistic regression', d: 'The classic default-risk model: the chance of a contract ending in repossession, explained by fines PER MONTH of contract (raw fine counts would punish long contracts) and the plate\'s km/week. Fitted by maximum likelihood (IRLS), same result as statsmodels\' logit. ODDS RATIO above 1 = each extra unit of the variable multiplies the odds of repossession by that much; p < 0,05 = the association is unlikely to be luck. The client SCORE will join the model as soon as the fleet-site API exposes it as a field. Note: the odds ratio here and the hazard ratio in Survival measure different things (chance of ever happening vs speed of happening), so their km/week numbers legitimately differ.' };
          // km entra em CENTENAS de km/semana — o odds ratio "por 1 km" arredonda para ×1,00 e não diz nada.
          // (contract age saiu do modelo: era circular — contrato recuperado é curto por construção;
          // entra o SCORE do cliente quando a API do site de Frota expuser o campo)
          const rowsL = AR.filter((r) => r.kmWeek != null).map((r) => ({ y: r.rec, finesPM: r.finesPM, kmWeek: r.kmWeek / 100 }));
          const fit = logisticFit(rowsL, ['finesPM', 'kmWeek']);
          const LBL = { finesPM: 'Fines / month', kmWeek: 'km/week (per +100)' };
          if (fit) {
            const ors = fit.coefs;
            mk('ccPerCar', { type: 'bar', data: { labels: ors.map((o) => LBL[o.k]),
                datasets: [{ data: ors.map((o) => Math.round(o.or * 100) / 100), backgroundColor: ors.map((o) => (o.p < .05 ? (o.or > 1 ? '#DC2626' : '#15803D') : '#9CA3AF')), borderRadius: 5, maxBarThickness: 46 }] },
              plugins: [{ id: 'orRef', beforeDatasetsDraw(ch) { const { ctx, chartArea: a } = ch, ys = ch.scales.y;
                const y1 = ys.getPixelForValue(1); ctx.save(); ctx.strokeStyle = '#374151'; ctx.setLineDash([5, 4]); ctx.beginPath(); ctx.moveTo(a.left, y1); ctx.lineTo(a.right, y1); ctx.stroke();
                ctx.setLineDash([]); ctx.font = '700 9px ' + getComputedStyle(document.body).fontFamily; ctx.fillStyle = '#6B7280'; ctx.textAlign = 'left'; ctx.fillText('odds ratio = 1 (no effect)', a.left + 4, y1 - 5); ctx.restore(); } }],
              options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 18 } },
                plugins: { legend: { display: false },
                  ...
                  tooltip: { displayColors: false, callbacks: { label: (c2) => { const o = ors[c2.dataIndex]; return `odds ratio ${o.or.toFixed(2)} · p ${fmtP(o.p)}`; } } } },
                scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5, weight: 700 }, color: '#374151', maxRotation: 0 } },
                  y: { beginAtZero: true, grace: '12%', grid: { display: false }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280' } } } } });
            if (statsEl) {
              const sig = ors.filter((o) => o.p < .05);
              statsEl.innerHTML =
                `<table><thead><tr><th>Variable</th><th>coef (β)</th><th>odds ratio</th><th>p-value</th></tr></thead><tbody>` +
                ors.map((o) => `<tr><td>${LBL[o.k]}</td><td>${o.beta.toFixed(3).replace('.', ',')}</td><td><b>×${o.or.toFixed(2).replace('.', ',')}</b></td><td class="${sigCls(o.p)}">${fmtP(o.p)}</td></tr>`).join('') +
                `</tbody></table>` +
                read(`<b>${fit.n}</b> contracts with km data · <b>${fit.events}</b> repossessions. ` +
                  (sig.length ? sig.map((o) => `<b>${LBL[o.k]}</b> ${o.or > 1 ? 'raises' : 'lowers'} the odds ×${o.or.toFixed(2).replace('.', ',')} per unit (p ${fmtP(o.p)})`).join('; ') + '.' : 'No variable clears p < 0,05 yet — with this sample, none of them separates who gets repossessed.'));
            }
          } else { mk('ccPerCar', { type: 'bar', data: { labels: [], datasets: [] }, options: {} }); if (statsEl) statsEl.innerHTML = read('Not enough data to fit the model yet.'); }
          return;
        }
        // ---------- 2) km/SEMANA: boxplot recuperados × não + Mann-Whitney ----------
        if (costsCorrTab === 'km') {
          costsLeftHelp = { t: 'km/week — recovered vs kept', d: 'Side-by-side boxplots of the plate\'s km/week for contracts that ended in repossession and the rest. Box = middle 50% (quartiles), line = median, whiskers = 5th–95th percentile. The MANN-WHITNEY test compares the two groups without assuming any distribution: p < 0,05 means the km/week of repossessed contracts is genuinely different, not sample luck. km/week is the plate\'s current pace — a proxy for contracts that already ended.' };
          const g1 = AR.filter((r) => r.rec && r.kmWeek != null).map((r) => r.kmWeek);
          const g0 = AR.filter((r) => !r.rec && r.kmWeek != null).map((r) => r.kmWeek);
          if (g1.length >= 3 && g0.length >= 3) {
            const q1 = quart(g1), q0 = quart(g0);
            const mw = mannWhitney(g1, g0);
            const QS = [q1, q0];
            // pontos individuais (strip plot) por cima das caixas: cada contrato vira uma bolinha
            // com jitter horizontal — dá pra VER a distribuição, não só o resumo
            const jit = (arr, cx, rows2) => arr.map((v, i2) => { const r = rows2[i2]; return { x: cx + (((i2 * 137) % 100) / 100 - .5) * .42, y: v, placa: r.placa, nome: r.nome }; });
            const rows1 = AR.filter((r) => r.rec && r.kmWeek != null), rows0 = AR.filter((r) => !r.rec && r.kmWeek != null);
            mk('ccPerCar', { type: 'bar', data: { labels: [`repossessed (${g1.length})`, `kept / other (${g0.length})`],
                datasets: [
                  { data: [[q1.q1, q1.q3], [q0.q1, q0.q3]], backgroundColor: ['rgba(220,38,38,.18)', 'rgba(107,114,128,.14)'], borderColor: ['#DC2626', '#6B7280'], borderWidth: 1.5, borderRadius: 6, maxBarThickness: 150, order: 3, datalabels: { display: false } },
                  { type: 'scatter', xAxisID: 'x2', data: jit(g1, 0, rows1), backgroundColor: 'rgba(220,38,38,.65)', pointRadius: 3, pointHoverRadius: 5, pointBorderColor: '#fff', pointBorderWidth: .5, order: 1, datalabels: { display: false } },
                  { type: 'scatter', xAxisID: 'x2', data: jit(g0, 1, rows0), backgroundColor: 'rgba(107,114,128,.45)', pointRadius: 3, pointHoverRadius: 5, pointBorderColor: '#fff', pointBorderWidth: .5, order: 2, datalabels: { display: false } },
                ] },
              plugins: [{ id: 'boxParts', afterDatasetsDraw(ch) {
                const { ctx } = ch, ys = ch.scales.y, meta = ch.getDatasetMeta(0);
                const fam = getComputedStyle(document.body).fontFamily;
                ctx.save();
                meta.data.forEach((el, i2) => { const q = QS[i2]; const w = el.width;
                  const col = i2 === 0 ? '#B91C1C' : '#4B5563';
                  ctx.strokeStyle = col; ctx.lineWidth = 3;                                    // mediana
                  ctx.beginPath(); ctx.moveTo(el.x - w / 2, ys.getPixelForValue(q.med)); ctx.lineTo(el.x + w / 2, ys.getPixelForValue(q.med)); ctx.stroke();
                  ctx.lineWidth = 1.2;                                                         // bigodes p5–p95
                  ctx.beginPath(); ctx.moveTo(el.x, ys.getPixelForValue(q.q3)); ctx.lineTo(el.x, ys.getPixelForValue(q.p95));
                  ctx.moveTo(el.x - w / 5, ys.getPixelForValue(q.p95)); ctx.lineTo(el.x + w / 5, ys.getPixelForValue(q.p95));
                  ctx.moveTo(el.x, ys.getPixelForValue(q.q1)); ctx.lineTo(el.x, ys.getPixelForValue(q.p5));
                  ctx.moveTo(el.x - w / 5, ys.getPixelForValue(q.p5)); ctx.lineTo(el.x + w / 5, ys.getPixelForValue(q.p5)); ctx.stroke();
                  // VALORES escritos: mediana em destaque à direita da caixa, quartis discretos
                  ctx.textBaseline = 'middle'; ctx.textAlign = 'left';
                  ctx.font = '800 12px ' + fam; ctx.fillStyle = col;
                  ctx.fillText(ccNum(q.med), el.x + w / 2 + 8, ys.getPixelForValue(q.med));
                  ctx.font = '600 9.5px ' + fam; ctx.fillStyle = '#9CA3AF';
                  ctx.fillText(ccNum(q.q3), el.x + w / 2 + 8, ys.getPixelForValue(q.q3));
                  ctx.fillText(ccNum(q.q1), el.x + w / 2 + 8, ys.getPixelForValue(q.q1));
                });
                ctx.restore();
              } }],
              options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 22, right: 8 } },
                plugins: { legend: { display: false }, datalabels: { display: false },
                  tooltip: { displayColors: false, callbacks: {
                    title: (items) => { const p = items[0].raw; return p && p.placa ? `${p.placa}${p.nome ? ' · ' + p.nome.split(' ').slice(0, 2).join(' ') : ''}` : ''; },
                    label: (c2) => { const p = c2.raw;
                      if (p && p.placa) return ccNum(p.y) + ' km/week';
                      const q = QS[c2.dataIndex]; return `median ${ccNum(q.med)} · box ${ccNum(q.q1)}–${ccNum(q.q3)} · whiskers ${ccNum(q.p5)}–${ccNum(q.p95)} km/wk`; } } } },
                scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 11.5, weight: 700 }, color: '#374151' } },
                  x2: { type: 'linear', display: false, min: -.5, max: 1.5 },
                  y: { beginAtZero: true, grace: '8%', grid: { color: 'rgba(120,120,140,.10)' }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: ccK }, title: { display: true, text: 'km/week', color: '#9CA3AF', font: { size: 10 } } } } } });
            if (statsEl) statsEl.innerHTML =
              `<table><thead><tr><th>Group</th><th>n</th><th>median</th><th>Q1–Q3</th></tr></thead><tbody>` +
              `<tr><td>Repossessed</td><td>${g1.length}</td><td><b>${ccNum(q1.med)}</b></td><td>${ccNum(q1.q1)}–${ccNum(q1.q3)}</td></tr>` +
              `<tr><td>Kept / other end</td><td>${g0.length}</td><td><b>${ccNum(q0.med)}</b></td><td>${ccNum(q0.q1)}–${ccNum(q0.q3)}</td></tr>` +
              `</tbody></table>` +
              read(`Mann-Whitney: U = ${Math.round(mw.U)}, p = <b class="${sigCls(mw.p)}">${fmtP(mw.p)}</b>. ` +
                (mw.p < .05 ? `Repossessed contracts run <b>${q1.med < q0.med ? 'less' : 'more'}</b> km/week — a real usage difference.` : 'No significant km/week difference between the groups so far.'));
          } else { if (statsEl) statsEl.innerHTML = read('Too few repossessions with km data for a group comparison.'); mk('ccPerCar', { type: 'bar', data: { labels: [], datasets: [] }, options: {} }); }
          return;
        }
        // ---------- 3) POR MODELO: tabela de contingência + qui-quadrado ----------
        if (costsCorrTab === 'model') {
          costsLeftHelp = { t: 'Repossession rate by car model', d: 'Do some car models get repossessed more? The bars are the repossession RATE per model (repossessions ÷ contracts of that model), and the chi-square test asks whether the differences between models are bigger than luck would produce (p < 0,05 = yes). Small groups make weak evidence — the table shows every n.' };
          const byM = {};
          AR.forEach((r) => { const m = r.model || '?'; (byM[m] = byM[m] || { n: 0, rec: 0 }).n++; if (r.rec) byM[m].rec++; });
          const ms = Object.entries(byM).filter(([, o]) => o.n >= 3).sort((a, b) => b[1].n - a[1].n);
          const totN = ms.reduce((s, [, o]) => s + o.n, 0), totR = ms.reduce((s, [, o]) => s + o.rec, 0);
          let chi = 0;
          ms.forEach(([, o]) => { const eR = o.n * totR / totN, eK = o.n * (totN - totR) / totN;
            if (eR > 0) chi += (o.rec - eR) * (o.rec - eR) / eR;
            if (eK > 0) chi += ((o.n - o.rec) - eK) * ((o.n - o.rec) - eK) / eK; });
          const df = ms.length - 1, pChi = df > 0 ? chi2P(chi, df) : null;
          mk('ccPerCar', { type: 'bar', data: { labels: ms.map(([m]) => m),
              datasets: [{ data: ms.map(([, o]) => Math.round(o.rec / o.n * 1000) / 10), backgroundColor: ms.map(([m]) => MODEL_COLOR[Object.keys(MODEL_COLOR).find((k2) => m.includes(k2))] || C), borderRadius: 5, maxBarThickness: 56 }] },
            options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 18 } },
              plugins: { legend: { display: false },
                datalabels: { anchor: 'end', align: 'top', offset: 1, color: '#374151', font: { size: 10.5, weight: 800 }, formatter: (v) => String(v).replace('.', ',') + '%' },
                tooltip: { displayColors: false, callbacks: { label: (c2) => { const o = ms[c2.dataIndex][1]; return `${o.rec} of ${o.n} contracts repossessed`; } } } },
              scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 10.5, weight: 700 }, color: '#374151', maxRotation: 0 } },
                y: { beginAtZero: true, grace: '12%', grid: { display: false }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: (v) => v + '%' } } } } });
          if (statsEl) statsEl.innerHTML =
            `<table><thead><tr><th>Model</th><th>contracts</th><th>repossessed</th><th>rate</th></tr></thead><tbody>` +
            ms.map(([m, o]) => `<tr><td>${escH(m)}</td><td>${o.n}</td><td>${o.rec}</td><td><b>${(o.rec / o.n * 100).toFixed(1).replace('.', ',')}%</b></td></tr>`).join('') +
            `</tbody></table>` +
            read(`Chi-square: χ² = ${chi.toFixed(2).replace('.', ',')} (df ${df}), p = <b class="${sigCls(pChi)}">${fmtP(pChi)}</b>. ` +
              (pChi != null && pChi < .05 ? 'The repossession rate really differs between models.' : 'The differences between models are within what chance would produce.'));
          return;
        }
        // ---------- 4) SOBREVIVÊNCIA: Kaplan-Meier + Cox ----------
        if (costsCorrTab === 'surv') {
          costsLeftHelp = { t: 'Survival — Kaplan-Meier + Cox', d: 'The curve is the Kaplan-Meier estimate of contracts SURVIVING repossession over their life: it starts at 100% and steps down at each repossession; contracts still running (or ended for other reasons) are censored, not counted as events. The Cox model below asks which variables ACCELERATE repossession: hazard ratio above 1 = the variable raises the instantaneous risk (a HR of 1,5 = 50% higher risk per extra unit), with its p-value.' };
          const rowsS = AR.map((r) => ({ t: r.durM, ev: r.rec ? 1 : 0, finesPM: r.finesPM, kmWeek: r.kmWeek != null ? r.kmWeek / 100 : null }));
          const kmC = kmSurvival(rowsS, 12);
          const rowsCox = rowsS.filter((r) => r.kmWeek != null);
          const cox = coxFit(rowsCox, ['finesPM', 'kmWeek']);
          const stepPts = [];   // curva em escada
          kmC.curve.forEach((pt, i2) => { if (i2 > 0) stepPts.push({ x: pt.t, y: kmC.curve[i2 - 1].s * 100 }); stepPts.push({ x: pt.t, y: pt.s * 100 }); });
          stepPts.push({ x: 12, y: kmC.curve[kmC.curve.length - 1].s * 100 });
          mk('ccPerCar', { type: 'line', data: { datasets: [{ label: 'contracts surviving repossession', data: stepPts, borderColor: C, backgroundColor: costsTint(C, .15), fill: true, stepped: false, pointRadius: 0, borderWidth: 2.5, tension: 0 }] },
            options: { responsive: true, maintainAspectRatio: false,
              plugins: { legend: { labels: CC_LEG }, datalabels: { display: false },
                tooltip: { displayColors: false, callbacks: { label: (c2) => `M${c2.parsed.x.toFixed(1)} · ${c2.parsed.y.toFixed(1).replace('.', ',')}% still with the driver` } } },
              scales: { x: { type: 'linear', min: 0, max: 12, grid: { display: false }, border: { display: false }, title: { display: true, text: 'months of contract', color: '#9CA3AF', font: { size: 10 } }, ticks: { font: CC_FONT, color: '#6B7280', stepSize: 1 } },
                y: { min: 0, max: 100, grid: { color: 'rgba(120,120,140,.10)' }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: (v) => v + '%' } } } } });
          if (statsEl) {
            const LBL2 = { finesPM: 'Fines / month', kmWeek: 'km/week (per +100)' };
            statsEl.innerHTML =
              (cox ? `<table><thead><tr><th>Cox variable</th><th>hazard ratio</th><th>p-value</th></tr></thead><tbody>` +
                cox.coefs.map((o) => `<tr><td>${LBL2[o.k]}</td><td><b>×${o.hr.toFixed(2).replace('.', ',')}</b></td><td class="${sigCls(o.p)}">${fmtP(o.p)}</td></tr>`).join('') +
                `</tbody></table>` : '') +
              read(`Kaplan-Meier over <b>${rowsS.length}</b> contracts (${rowsS.filter((r) => r.ev).length} repossessions): ` +
                `by M3 <b>${(kmSurvival(rowsS, 3).curve.slice(-1)[0].s * 100).toFixed(1).replace('.', ',')}%</b> of contracts still stand. ` +
                (cox ? cox.coefs.filter((o) => o.p < .05).map((o) => `<b>${LBL2[o.k]}</b> multiplies the repossession hazard ×${o.hr.toFixed(2).replace('.', ',')} per unit (p ${fmtP(o.p)})`).join('; ') + (cox.coefs.some((o) => o.p < .05) ? '.' : 'No Cox variable clears p < 0,05 yet.') : '') +
                ' The HAZARD ratio here is not the same as the Risk model\'s odds ratio: Cox measures how much the variable ACCELERATES repossession over time, the logit measures the chance of it ever happening — close, but legitimately different numbers.');
          }
          return;
        }
        // ---------- 5) 2ª SEMANA sem pagar × recuperação ----------
        if (costsCorrTab === 'pay2') {
          costsLeftHelp = { t: 'The 2nd week tells the story', d: 'The 1st week is paid on pickup — the 2nd is the first one a driver can actually miss. Bars: repossession rate among contracts whose 2nd weekly payment was LATE or never made (the payments matrix flags late payments and omits unpaid weeks) vs the ones that paid it on time. Contracts younger than 3 weeks are left out — their 2nd week has not happened yet. The chi-square p says whether the gap is real. A big gap makes the 2nd week the earliest tripwire we have.' };
          const rowsP = AR.filter((r) => r.delay2 != null);
          const gMiss = rowsP.filter((r) => r.delay2), gPaid = rowsP.filter((r) => !r.delay2);
          const rMiss = gMiss.filter((r) => r.rec).length, rPaid = gPaid.filter((r) => r.rec).length;
          let chi2 = 0, p2 = null;
          if (gMiss.length && gPaid.length) {
            const N = rowsP.length, R = rMiss + rPaid;
            [[gMiss.length, rMiss], [gPaid.length, rPaid]].forEach(([n, r]) => {
              const eR = n * R / N, eK = n * (N - R) / N;
              if (eR > 0) chi2 += (r - eR) * (r - eR) / eR;
              if (eK > 0) chi2 += ((n - r) - eK) * ((n - r) - eK) / eK;
            });
            p2 = chi2P(chi2, 1);
          }
          mk('ccPerCar', { type: 'bar', data: { labels: [`2nd week late / unpaid (${gMiss.length})`, `2nd week on time (${gPaid.length})`],
              datasets: [{ data: [gMiss.length ? Math.round(rMiss / gMiss.length * 1000) / 10 : 0, gPaid.length ? Math.round(rPaid / gPaid.length * 1000) / 10 : 0],
                backgroundColor: ['#DC2626', '#9CA3AF'], borderRadius: 5, maxBarThickness: 92 }] },
            options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 18 } },
              plugins: { legend: { display: false },
                datalabels: { anchor: 'end', align: 'top', offset: 1, color: '#374151', font: { size: 12, weight: 800 }, formatter: (v) => String(v).replace('.', ',') + '%' },
                tooltip: { displayColors: false, callbacks: { label: (c2) => (c2.dataIndex === 0 ? `${rMiss} of ${gMiss.length} repossessed` : `${rPaid} of ${gPaid.length} repossessed`) } } },
              scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: { size: 11, weight: 700 }, color: '#374151' } },
                y: { beginAtZero: true, grace: '12%', grid: { display: false }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: (v) => v + '%' }, title: { display: true, text: 'repossession rate', color: '#9CA3AF', font: { size: 10 } } } } } });
          if (statsEl) statsEl.innerHTML =
            `<table><thead><tr><th>2nd weekly payment</th><th>contracts</th><th>repossessed</th><th>rate</th></tr></thead><tbody>` +
            `<tr><td>Late or never paid</td><td>${gMiss.length}</td><td>${rMiss}</td><td><b>${gMiss.length ? (rMiss / gMiss.length * 100).toFixed(1).replace('.', ',') : '—'}%</b></td></tr>` +
            `<tr><td>Paid on time</td><td>${gPaid.length}</td><td>${rPaid}</td><td><b>${gPaid.length ? (rPaid / gPaid.length * 100).toFixed(1).replace('.', ',') : '—'}%</b></td></tr>` +
            `</tbody></table>` +
            read(`Chi-square: χ² = ${chi2.toFixed(2).replace('.', ',')}, p = <b class="${sigCls(p2)}">${fmtP(p2)}</b>. ` +
              (p2 != null && p2 < .05 ? 'A late 2nd week is a REAL early-warning sign — worth acting on it immediately.' : 'The gap is not statistically solid yet, but watch it as the sample grows.') +
              ' Contracts younger than 3 weeks are left out.');
          return;
        }
        ...
        if (costsCorrTab === 'drop') {
          ...
          if (!KS || !KS.weeks || !KS.weeks.length) {
            ...
            if (statsEl) statsEl.innerHTML = read('Odometer history not available on this refresh — the fleet-site readings feed did not load.');
            return;
          }
          const wIdx = (iso) => { const w = KS.weeks; for (let i2 = w.length - 1; i2 >= 0; i2--) if (w[i2] <= iso) return i2; return -1; };
          // média de km por semana RELATIVA à recuperação (t-8..t0), só semanas dentro do contrato
          const REL = 8, sum = new Array(REL + 1).fill(0), cnt = new Array(REL + 1).fill(0);
          AR.filter((r) => r.rec && r.fim).forEach((r) => {
            const serie = KS.placas[r.placa]; if (!serie) return;
            const i0 = wIdx(r.fim); if (i0 < 0) return;
            for (let k2 = 0; k2 <= REL; k2++) {
              const i2 = i0 - k2; if (i2 < 0) break;
              if (KS.weeks[i2] < r.ini) break;                    // fora do contrato
              const v = serie[i2]; if (v == null) continue;
              sum[REL - k2] += v; cnt[REL - k2]++;
            }
          });
          const avgRel = sum.map((s, i2) => (cnt[i2] ? Math.round(s / cnt[i2]) : null));
          // referência: ritmo médio das placas de contratos ATIVOS nas últimas 4 semanas completas
          const activePl = new Set(AR.filter((r) => !r.fim).map((r) => r.placa));
          let aSum = 0, aCnt = 0;
          activePl.forEach((pl) => { const s = KS.placas[pl]; if (!s) return;
          ...
          mk('ccPerCar', { type: 'line', data: { labels: labelsRel, datasets: [
            ...
            options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 24 } },
              plugins: { legend: { labels: CC_LEG },
                tooltip: { displayColors: false, callbacks: { label: (c2) => ccNum(c2.parsed.y) + ' km/week · avg of ' + (cnt[c2.dataIndex] || 0) + ' repossessed contracts with data' } } },
              scales: { x: { grid: { display: false }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280' } },
                y: { beginAtZero: true, grace: '10%', grid: { color: 'rgba(120,120,140,.10)' }, border: { display: false }, ticks: { font: CC_FONT, color: '#6B7280', callback: ccK }, title: { display: true, text: 'average km per week', color: '#9CA3AF', font: { size: 10 } } } } } });
          // watchlist: contratos ATIVOS com a última semana completa < 50% da média das 3 anteriores.
          // Contrato precisa de pelo menos 28 dias E as semanas-base DENTRO do próprio contrato —
          // senão a "base" vem do motorista anterior da placa (caso real: contrato de 3 dias).
          const watch = [];
          AR.filter((r) => !r.fim && r.durM >= 28 / 30.44).forEach((r) => {
            const s = KS.placas[r.placa]; if (!s) return;
            const iLast = KS.weeks.length - 2;                    // última semana COMPLETA
            if (iLast < 3) return;
            if (KS.weeks[iLast - 3] < r.ini) return;              // base sairia de fora do contrato
            const last = s[iLast]; if (last == null) return;
            const prev = [s[iLast - 1], s[iLast - 2], s[iLast - 3]].filter((v) => v != null);
      ...
      if (costsSel === 'Repair cost') {
        const JB = (((OCN.ue || {}).judBase) || {}).placas || {};
        const pts = [];
        Object.entries(JB).forEach(([pl, a]) => a.forEach((c2) => {
          if (!(c2.repair > 0)) return;
          const d = c2.d || hojeIso;
          pts.push({ x: monthFrac(d), y: Math.round(((c2.repair || 0) / fx) * K), placa: pl, d });
        }));
        const isRec = false;
        if (!pts.length) { show(false); return; }
        show(true); title('Cost per repair');
        costsLeftHelp = { t: 'Cost per repair', d: 'One dot per repair case from the judicial base (repair + cleaning + others), on the date of the event. The dashed line is the average cost per case — the "expected cost" of the next one.' };
    ...
    // ---- km/semana da FROTA no tempo (overview) ----
    // A média semanal de todas as placas com leitura. A suspeita que motivou o gráfico: recuperamos
    // carros de quem roda POUCO e recolocamos com motoristas médios — então a média da frota sobe
    // com o churn. A linha tracejada projeta o ritmo das últimas 8 semanas 4 semanas à frente.
    // O toggle 'Pattern breaks' vira a lente para o INDIVÍDUO: placas cuja média das 2 últimas
    // semanas caiu mais de 60% contra as 4 anteriores — o desenho clássico de carro sumido do uso.
    function renderKmTrend() {
      const mk = (id, cfg) => { if (costsCharts[id]) { costsCharts[id].destroy(); delete costsCharts[id]; } const c = document.getElementById(id); if (!c) return; costsCharts[id] = new Chart(c.getContext('2d'), cfg); };
      const cnv = document.getElementById('ccKmTrend'); if (!cnv) return;
      const KS = (OCN.ue || {}).kmSemanal;
      const box = cnv.closest('.costs-chart');
      if (!KS || !KS.weeks || !KS.weeks.length) { if (box) box.style.display = 'none'; return; }
      if (box) box.style.display = '';
      const MUT = () => (document.documentElement.classList.contains('dark') ? '#9AA0AC' : '#737373');
      const GRD = () => (document.documentElement.classList.contains('dark') ? 'rgba(255,255,255,.07)' : '#e5e5e5');
      const INKC = () => (document.documentElement.classList.contains('dark') ? '#E7E8EC' : '#0a0a0a');
      const ctl = document.getElementById('ccKmCtl'), tit = document.getElementById('ccKmT'), hint = document.getElementById('ccKmHint');
      if (ctl) {
        ctl.innerHTML = [['trend', 'Fleet average'], ['breaks', 'Pattern breaks']].map(([v, t]) =>
          `<button type="button" class="ccl-btn cc-mini${costsKmView === v ? ' on' : ''}" data-v="${v}">${t}</button>`).join('');
        ctl.querySelectorAll('.ccl-btn').forEach((b) => b.addEventListener('click', () => { costsKmView = b.dataset.v; renderKmTrend(); }));
      }
      const weeks = KS.weeks, series = Object.entries(KS.placas || {});
      if (costsKmView === 'trend') {
        if (tit) tit.textContent = 'Fleet km/week over time';
        // média por semana entre as placas COM leitura; a semana corrente (parcial) fica de fora
        const med = weeks.map((w, i) => {
          let s = 0, n = 0;
          series.forEach(([, arr]) => { const v = arr[i]; if (v != null && v > 0) { s += v; n++; } });
          return { w, v: n >= 10 ? s / n : null, n };
        }).filter((x) => x.v != null);
        const corpo = med.slice(0, -1);                              // última semana é parcial
        if (corpo.length < 4) { if (box) box.style.display = 'none'; return; }
        // tendência das últimas 8 semanas, projetada 4 à frente
        const cauda = corpo.slice(-8);
        const nn = cauda.length, sx = cauda.reduce((s, _, i) => s + i, 0), sy = cauda.reduce((s, q) => s + q.v, 0);
        const sxx = cauda.reduce((s, _, i) => s + i * i, 0), sxy = cauda.reduce((s, q, i) => s + i * q.v, 0);
        const den = nn * sxx - sx * sx;
        const m = den ? (nn * sxy - sx * sy) / den : 0, b0 = (sy - m * sx) / nn;
        const labels = corpo.map((x) => x.w.slice(5));
        const PROJ = 4;
        for (let k = 1; k <= PROJ; k++) { const d0 = new Date(corpo[corpo.length - 1].w + 'T12:00:00'); d0.setDate(d0.getDate() + 7 * k); labels.push(d0.toISOString().slice(5, 10)); }
        const linha = corpo.map((x) => Math.round(x.v));
        const proj = corpo.map((x, i) => (i === corpo.length - 1 ? Math.round(x.v) : null));
        for (let k = 1; k <= PROJ; k++) proj.push(Math.round(b0 + m * (nn - 1 + k)));
        if (hint) hint.textContent = 'trend of the last 8 weeks: ' + (m >= 0 ? '+' : '−') + Math.abs(m).toFixed(0) + ' km/week per week · dashed = 4-week projection';
        mk('ccKmTrend', { type: 'line', data: { labels, datasets: [
            { label: 'fleet average', data: linha.concat(Array(PROJ).fill(null)), borderColor: INKC(), backgroundColor: 'transparent',
              borderWidth: 2, pointRadius: 0, pointHoverRadius: 4, tension: .25 },
            { label: 'projection', data: proj, borderColor: MUT(), borderDash: [5, 4], borderWidth: 1.5, pointRadius: 0, tension: 0 },
          ] },
          options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
            plugins: { legend: { display: false }, datalabels: { display: false },
              tooltip: { displayColors: false, callbacks: { label: (c2) => (c2.datasetIndex ? 'projected ' : '') + ccNum(c2.parsed.y) + ' km/week',
                afterBody: (items) => { const q = med[items[0].dataIndex]; return q ? q.n + ' cars reporting' : ''; } } } },
            scales: { x: { grid: { display: false }, border: { display: false }, ticks: { color: MUT(), font: { size: 10 }, maxTicksLimit: 12 } },
              y: { border: { display: false }, grid: { color: GRD(), drawTicks: false },
                ticks: { color: MUT(), font: { size: 11 }, padding: 6, callback: ccK } } } } });
        return;
      }
      // ---- quebras de padrão por placa ----
      if (tit) tit.textContent = 'Usage pattern breaks';
      const quedas = [];
      series.forEach(([pl, arr]) => {
        // ignora a última semana (parcial): compara [ -6..-3 ) com [ -3..-1 )
        const a4 = arr.slice(-7, -3).filter((v) => v != null), r2 = arr.slice(-3, -1).filter((v) => v != null);
        if (a4.length < 3 || r2.length < 1) return;
        const antes = a4.reduce((s, v) => s + v, 0) / a4.length;
        const agora = r2.reduce((s, v) => s + v, 0) / r2.length;
        if (antes >= 400 && agora < antes * 0.4) quedas.push({ pl, antes: Math.round(antes), agora: Math.round(agora), drop: 1 - agora / antes });
      });
      quedas.sort((a, b) => b.drop - a.drop);
      const topQ = quedas.slice(0, 14);
      if (hint) hint.textContent = quedas.length + ' car' + (quedas.length === 1 ? '' : 's') + ' dropped >60% vs their own 4-week pace — the classic shape of a car slipping out of use';
      if (!topQ.length) {
        mk('ccKmTrend', { type: 'bar', data: { labels: ['no pattern break'], datasets: [{ data: [0] }] },
          options: { plugins: { legend: { display: false }, datalabels: { display: false } } } });
        return;
      }
      mk('ccKmTrend', { type: 'bar', data: { labels: topQ.map((q) => q.pl), datasets: [
          { label: 'before', data: topQ.map((q) => q.antes), backgroundColor: GRD(), borderRadius: 3, maxBarThickness: 26 },
          { label: 'now', data: topQ.map((q) => q.agora), backgroundColor: 'rgba(220,38,38,.75)', borderRadius: 3, maxBarThickness: 26 },
        ] },
        options: { responsive: true, maintainAspectRatio: false,
          plugins: { legend: { labels: { color: MUT(), boxWidth: 10, font: { size: 10.5 } } },
            datalabels: { display: false },
            tooltip: { displayColors: false, callbacks: {
              title: (items) => topQ[items[0].dataIndex].pl,
              label: (c2) => (c2.datasetIndex ? 'last 2 weeks: ' : '4-week pace before: ') + ccNum(c2.parsed.y) + ' km/week',
              footer: (items) => '−' + Math.round(topQ[items[0].dataIndex].drop * 100) + '% — worth a look' } } },
          scales: { x: { grid: { display: false }, border: { display: false }, ticks: { color: MUT(), font: { size: 9.5 }, maxRotation: 45, minRotation: 45 } },
            y: { border: { display: false }, grid: { color: GRD(), drawTicks: false }, ticks: { color: MUT(), font: { size: 11 }, callback: ccK } } } } });
    }
    function renderCosts() {
      ...
      if (isMain) { renderCostsMain(P, { S, K, cs, money, lineOfL, sumOf, mSel, inScope }); renderKmTrend(); return; }
        ...
        if (costsSel !== 'Insurance') { insBox.innerHTML = ''; insBox.style.display = 'none'; } else {
          ...
          insBox.innerHTML =
            `<div class="ins-panel" style="--cl:${C}">` +
              ...
              `</div>` +
              (() => { const IP = insParcelasRS(rfFleets.map((f2) => f2.id)), fxr = finPar('__fin_fx__') || 5.5;
                return `<div class="ins-basis">Premium <b>accrued</b> for ${escH(perLbl)}: <b>${money(accrued)}</b>` +
                `<span>·</span>disbursed in cash by the same fleets: <b>${money(cash)}</b>` +
                (IP.depois > 0 ? `<span>·</span>due in ${finYear + 1}: <b>${money(IP.depois / fxr)}</b><span>·</span>full contracts: <b>${money(IP.total / fxr)}</b>` : '') +
          ...
          casesBox.innerHTML =
            `<div class="ccc-panel" style="--cl:${C}">` +
              `<div class="cc-head"><h4>${costsSel === 'Recovery cost' ? 'Recovery cases — plate by plate' : 'Repair cases — plate by plate'}</h4>` +
                `<span class="cc-hint">${rowsC.length} case${rowsC.length === 1 ? '' : 's'} with realized cost in ${escH(perName(mSel))} · ${money(totC2)} total${rowsC.length ? ' · ' + money(totC2 / rowsC.length) + ' per case' : ''}</span>` +
                (rowsC.length ? `<button type="button" class="ccl-btn" id="cccToggle" style="--c:${C}">${costsCasesOpen ? 'Hide table' : 'Show table (' + rowsC.length + ')'}</button>` : '') + `</div>` +
              `<div class="ccc-months">${monthsRow || '<span class="ccc-mo">no cases in the period</span>'}</div>` +
              // a tabela nasce RECOLHIDA — abre só no botão, pra página não ficar quilométrica
              (rowsC.length && costsCasesOpen ? `<div class="ccc-tblwrap"><table class="ccc-tbl"><thead><tr><th>Plate</th><th>Fleet</th><th>Date</th><th>Car age</th><th>Cost</th></tr></thead><tbody>` +
                ...
                `</tbody></table></div>` : '') +
            `</div>`;
          const tg = document.getElementById('cccToggle');
          if (tg) tg.addEventListener('click', () => { costsCasesOpen = !costsCasesOpen; renderCosts(); });
        }
      }
      ...
      if (costsSel === 'Recovery cost') {
        ...
        const hojeIso2 = (OCN.ue && OCN.ue.hoje) || new Date().toISOString().slice(0, 10);
        const scIso = (iso) => { const d = iso || hojeIso2; return String(d).slice(0, 4) === String(finYear) && inScope(parseInt(String(d).slice(5, 7), 10) - 1); };
        const evsIn = RD.evs.filter((e) => scIso(e.fim));
        // custo só das RECUPERAÇÕES (para a média por recuperação)...
        const costRecOnly = RD.cases.filter((c2) => scIso(c2.d)).reduce((s, c2) => s + c2.guincho + c2.recup, 0) / RD.fx;
        // ...mas o cartão TOTAL usa a MESMA régua da linha do COGS (fy = período do calendário,
        // incluindo guincho/recuperação de devoluções e perda total + forecast no ano cheio) —
        // pedido: os números têm que bater com o "Where it sits in COGS"
        const costIn = fy;
        const nRec = evsIn.length;
        // vida ESPERADA do contrato por Kaplan-Meier: evento = contrato encerrado (recuperação OU
        // devolução OU perda; troca de carro censura), contratos VIGENTES entram como censura —
      ...
      const MONL = Array.from({ length: nMes }, (_, m) => monthLbl(m));
      const cut12 = (a) => a.slice(0, nMes);
      const noDL = { datalabels: { display: false } };
      // ---- caixa da ESQUERDA: um gráfico PRÓPRIO de cada linha (substituiu o per-car/elasticidade) ----
      renderCostsLeft(C, { mk, K, cs, noDL, money });
      if (costsSel === 'Recovery cost') {
        // ---- composição das RECUPERAÇÕES por mês: com guincho + recuperação · só guincho · sem custo ----
        document.getElementById('ccMonT').textContent = 'Repossessions per month';
        const RD3 = recoveryData();
        const zBoth = new Array(FIN_MONTHS).fill(0), zTow = new Array(FIN_MONTHS).fill(0), zNone = new Array(FIN_MONTHS).fill(0);
        RD3.evs.forEach((e) => {
          if (String(e.fim).slice(0, 4) !== String(finYear)) return;
          const m = parseInt(String(e.fim).slice(5, 7), 10) - 1;
          if (m < 0 || m >= FIN_MONTHS) return;
      ...
      document.getElementById('ccMonT').textContent = 'Monthly cost';
      mk('ccMain', { data: { labels: MONL, datasets: [
          { type: 'bar', label: costsSel, data: cut12(arr.map((v) => Math.round(v * K))), backgroundColor: cut12(arr.map((_, m) => (mSel != null ? (inScope(m) ? C : costsTint(C, .28)) : C))), yAxisID: 'y', borderRadius: 3, maxBarThickness: 30 },
          { type: 'line', label: 'Active cars', data: cut12(act.map((v) => Math.round(v))), borderColor: '#EB6834', backgroundColor: 'transparent', yAxisID: 'y2', tension: .3, pointRadius: 2, borderWidth: 2 },
      ...
      mk('ccRank', { type: 'bar', data: { labels: rankRows.map((r) => COSTS_LABEL(r.L)), datasets: [{ data: rankRows.map((r) => Math.round(r.v * K)), backgroundColor: rankRows.map((r) => r.L === costsSel ? COSTS_COLOR[r.L] : costsTint(COSTS_COLOR[r.L], .35)), borderRadius: 4, maxBarThickness: 24 }] },
        options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false,
          onClick: (e, els) => { if (els && els.length) { costsSel = rankRows[els[0].index].L; costsWhatifPct = 0; renderCosts(); } },
          onHover: (e, els) => { e.native.target.style.cursor = els && els.length ? 'pointer' : 'default'; },
          plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'end', color: '#4B5563', font: { size: 10.5, weight: 700 }, formatter: ccK },
            tooltip: { displayColors: false, callbacks: { label: (c) => cs + ' ' + ccNum(c.parsed.x) + ' in the period', footer: () => 'Click to open this line' } } },
      ...
      // ---- 2º gráfico grande: perfil por idade OU visão POR FROTA ----
      // Em multas/recuperação/reparo o "quando bate na vida do carro" não conta a história certa
      // (multa não tem idade típica; recuperação/reparo têm amostra curta) — nessas linhas a caixa
      // vira "qual frota gera esse custo", por carro·mês, comparável entre frotas.
      const ageBox = document.getElementById('ccAgeBox');
      const inds = document.getElementById('ccAgeInds');
      const BYFLEET = { 'Traffic fines (out)': 1, 'Repair cost': 1 };
      costsAgeHelp2 = null;
      if (costsSel === 'Recovery cost') {
        // ---- composição dos CUSTOS de recuperação: guincho × recuperação, por mês ----
        ...
        mk('ccAge', { type: 'bar', data: { labels: MONL, datasets: [
          ...
          plugins: [{ id: 'compTot', afterDatasetsDraw(ch) {
            const { ctx } = ch; const tots = {}, top = {};
            ch.data.datasets.forEach((ds, di) => { const meta = ch.getDatasetMeta(di);
              meta.data.forEach((el, i2) => { const v = ds.data[i2]; if (!v) return; tots[i2] = (tots[i2] || 0) + v; top[i2] = Math.min(top[i2] == null ? Infinity : top[i2], el.y); }); });
            ctx.save(); ctx.font = '800 10.5px ' + getComputedStyle(document.body).fontFamily; ctx.fillStyle = '#374151'; ctx.textAlign = 'center';
            Object.keys(tots).forEach((i2) => { if (tots[i2] > 0) ctx.fillText(ccK(tots[i2]), ch.getDatasetMeta(0).data[i2].x, top[i2] - 5); });
            ctx.restore();
          } }],
          options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 18 } },
            plugins: { legend: { labels: CC_LEG },
        ...
        // multas: um botão de UMA palavra alterna a régua — por carro·mês ou o total do período
        const ageCtl = document.getElementById('ccAgeCtl');
        if (ageCtl) {
          if (costsSel === 'Traffic fines (out)') {
            ageCtl.innerHTML = `<button type="button" class="ccl-btn cc-mini${costsFleetTot ? ' on' : ''}" title="${costsFleetTot ? 'Showing the fleet TOTAL for the period — click for per car · month' : 'Showing per car · month — click for the fleet total'}">Total</button>`;
            ageCtl.firstChild.addEventListener('click', () => { costsFleetTot = !costsFleetTot; renderCosts(); });
          } else { ageCtl.innerHTML = ''; costsFleetTot = false; }
        }
        ...
        const cntByFleet = {};
        if (costsSel !== 'Traffic fines (out)') {
          const fld = costsSel === 'Recovery cost' ? 'recovery' : 'repair';
          Object.entries((((OCN.ue || {}).judBase) || {}).placas || {}).forEach(([pl, a]) => a.forEach((c2) => {
            if (c2[fld] > 0) cntByFleet[fofP[pl]] = (cntByFleet[fofP[pl]] || 0) + 1;
          }));
        } else {
        ...
        mk('ccAge', { type: 'bar', data: { labels: rowsBF.map((r) => 'Fleet ' + r.id + (bfProj && r.real <= 0 ? ' *' : '')),
            datasets: [{ data: rowsBF.map((r) => Math.round((costsFleetTot ? r.tot : r.per) * K)), backgroundColor: rowsBF.map((r, i2) => costsTint('#5A00F8', Math.max(.25, .95 - i2 * .13))), borderRadius: 4, maxBarThickness: 58 }] },
          options: { responsive: true, maintainAspectRatio: false, layout: { padding: { top: 20 } },
            plugins: { legend: { display: false },
              ...
              tooltip: { padding: 10, displayColors: false, callbacks: { label: (c2) => ccNum(c2.parsed.y) + (costsFleetTot ? ' total in the period' : ' per car · month'), afterBody: (items) => {
        ...
        const timing = fli == null ? null : (fli >= 1.4
          ? { t: 'Front-loaded', c: '#DC2626', d: 'concentrated at delivery — drains cash while the fleet ramps' }
          : (fli >= 0.8
            ? { t: 'Evenly spread', c: '#6B7280', d: 'spread evenly over the contract — neutral for cash' }
            : { t: 'Back-loaded', c: '#059669', d: 'lands at the end of the contract — the cash stays with us longer' }));
        if (inds) inds.innerHTML = timing == null ? '' :
    ...
    function unitData() {
      ...
      (U.fleets || []).forEach((f) => {
        ...
        // orçado do CONTRATO INTEIRO (M0..M13): base do retorno esperado no fim do ciclo
        let budFull = 0;
        for (let p = 0; p <= 13; p++) budFull += bm(p);
        budFull /= fx;
    ...
    function renderUnit() {
      ...
      const BUD = (r) => (unitFull ? (r.budFull || 0) : (r.bud || 0));
      const DLT = (r) => RET(r) - BUD(r);
      // ---- % DO ORÇADO: 1 + (real − orçado) / |orçado| ----
      // É a fórmula pedida ((1000 − 800) / 800 + 1 = 125%) com UMA mudança: o denominador é o
      // MÓDULO do orçado. Assim o sinal do desvio sempre quer dizer a mesma coisa — acima de 100%
      // é melhor que o plano, abaixo é pior — mesmo quando o plano é negativo (um carro novo, no
      // "So far", tem orçado negativo: perdeu 500 contra plano de −800 é 137%, MELHOR que o plano;
      // dividir por −800 daria 62% e leria ao contrário). Real negativo contra plano positivo dá
      // abaixo de 0%: o carro devolveu menos que nada do que deveria.
      // Orçado quase zero não dá razão (qualquer real vira infinito): fica de fora quando o
      // |orçado| não chega a 5% da mediana dos |orçados| do recorte, e a legenda conta quantos.
      const pctFloor = (() => {
        const abs = rows.map((r) => Math.abs(BUD(r))).filter((v) => v > 0).sort((a, b) => a - b);
        return abs.length ? abs[Math.floor(abs.length / 2)] * 0.05 : 0;
      })();
      const PCT = (r) => { const b = Math.abs(BUD(r)); return b > pctFloor && b > 0 ? 1 + DLT(r) / b : null; };
      const sorters = {
        ...
        pct: (a, b) => (PCT(b) == null ? -1e9 : PCT(b)) - (PCT(a) == null ? -1e9 : PCT(a)),   // melhor % do orçado primeiro
      };
      ...
      // as PLACAS entram no mesmo dropdown (grupo "Unitary"): com 170 barras de 9px, acertar a
      // do carro certo no clique é difícil — e um segundo seletor só para isso estourava a linha
      const cortes = [
        ['', 'All cars', null],
        ...
        ...all.slice().sort((a, b) => String(a.pl).localeCompare(String(b.pl)))
          .map((r) => ['p:' + r.pl, r.pl + ' · ' + (r.driver ? r.driver.split(' ').slice(0, 2).join(' ') : 'no driver'), 'Unitary']),
      ];
      const cortAtual = unitPlateSel ? 'p:' + unitPlateSel : (unitModel != null ? 'm:' + unitModel : (unitFleet != null ? 'f:' + unitFleet : ''));
      ...
      ctl.innerHTML = '<div class="costs-bar unit-bar">' +
      ...
      ctl.querySelector('#unitFleetSel').addEventListener('change', (e) => {
        ...
        if (v.startsWith('p:')) {
          // abrir uma placa: se ela não está no recorte em tela, o recorte cai — senão a barra
          // aberta não apareceria em lugar nenhum
          unitPlateSel = v.slice(2);
          if (!rows.some((r) => r.pl === unitPlateSel)) { unitFleet = null; unitModel = null; unitFilter = null; }
          renderUnit(); return;
        }
      ...
      if (viewWrap) {
        viewWrap.className = 'unit-seg unit-seg-eq';
      ...
      if (sortWrap) {
        const SORTS = [['delta', 'Biggest gap vs budget'], ['pct', 'Best % of budget'], ['real', 'Lowest return'], ['fleet', 'By fleet'], ['model', 'By model'], ['age', 'Oldest first'], ['kmwk', 'Highest km/week'], ['d15', 'Δ last 15 days']];
        const atual = SORTS.find((x) => x[0] === unitSort) || SORTS[0];
        sortWrap.className = 'ue-dd ue-dd-sort';
        sortWrap.innerHTML =
          `<button type="button" class="ue-dd-btn" aria-haspopup="listbox" aria-expanded="false">` +
            `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round"><path d="M7 4v16M7 20l-3-3M7 20l3-3M17 20V4M17 4l-3 3M17 4l3 3"/></svg>` +
            `<span>${atual[1]}</span>` +
          ...
          SORTS.map(([v, t]) => `<button type="button" class="ue-dd-opt${unitSort === v ? ' sel' : ''}" data-v="${v}"><span>${t}</span></button>`).join('') +
          `</div>`;
        const sBtn = sortWrap.querySelector('.ue-dd-btn'), sLista = sortWrap.querySelector('.ue-dd-list');
        const sFecha = () => { sLista.hidden = true; sBtn.setAttribute('aria-expanded', 'false'); };
        sBtn.addEventListener('click', (ev) => { ev.stopPropagation(); const abre = sLista.hidden; sLista.hidden = !abre; sBtn.setAttribute('aria-expanded', String(abre)); });
        sLista.querySelectorAll('.ue-dd-opt').forEach((o) => o.addEventListener('click', () => { sFecha(); unitSort = o.dataset.v; renderUnit(); }));
        document.addEventListener('click', sFecha);
        sortWrap.addEventListener('click', (ev) => ev.stopPropagation());
      }
      ...
      const pcts = rows.map(PCT).filter((v) => v != null).sort((a, b) => a - b);
      const medPct = pcts.length ? pcts[Math.floor(pcts.length / 2)] : null;
      const pctTxt = (v) => (v == null ? '—' : Math.round(v * 100).toLocaleString('pt-BR') + '%');
      const kpi = (t, v, sub, cls) => `<div class="unit-kpi"><span>${escH(t)}</span><b class="${cls || ''}">${v}</b><span class="sub">${escH(sub || '')}</span></div>`;
      document.getElementById('unitHero').innerHTML = '<div class="unit-kpis">' +
        kpi('Cars', String(rows.length),
          (unitModel != null ? unitModel : unitFleet != null ? 'fleet ' + unitFleet : 'all fleets') +
          (unitFilter === 'back' ? ' · back in' : unitFilter === 'below' ? ' · below budget' : '')) +
        kpi('Median vs budget', pctTxt(medPct), unitFull ? 'full contract' : 'at each car\'s age', medPct == null ? '' : (medPct >= 1 ? 'up' : 'bl')) +
        kpi('Above budget', String(above), Math.round(above / Math.max(1, rows.length) * 100) + '% of the cars', 'up') +
        kpi('Below budget', String(below), Math.round(below / Math.max(1, rows.length) * 100) + '% of the cars', 'bl') +
        kpi('Cash-positive', String(positive), 'return ≥ 0') +
        kpi('Total Δ vs budget', (totD >= 0 ? '+' : '−') + money(Math.abs(totD)), 'sum of the deltas', totD >= 0 ? 'up' : 'dn') +
        '</div>';
      ...
      const medRet = rows.length ? rows.reduce((s, r) => s + (modoFull ? (r.retFull || 0) : r.real), 0) / rows.length : 0;
      document.getElementById('unitRetHint').innerHTML = (unitSort === 'd15'
        ...
        `</span>`);
      // a legenda do orçado é um interruptor: clicou, a linha some (e volta)
      ...
      // ================= PERFORMANCE VS BUDGET (principal) =================
      // Mesmas barras, mesma ordem, mesmo clique — mas cada carro como % do PRÓPRIO orçado, o que
      // torna carros de frotas e idades diferentes comparáveis entre si. A barra é limitada a
      ...
      document.getElementById('unitPctHint').innerHTML = modo15
        ...
        : `<span class="cc-leg">` +
          `<i class="cc-leg-i"><span class="sw sw-up"></span>≥ 100% · ahead of its plan</i>` +
          `<i class="cc-leg-i"><span class="sw sw-bl"></span>0–100% · behind its plan</i>` +
          `<i class="cc-leg-i" title="The shortfall is bigger than the whole budget"><span class="sw sw-dn"></span>&lt; 0% · missed by more than the plan itself</i>` +
      ...
      const corPct = (ctx) => {
        const r = rows[ctx.dataIndex]; if (!r) return '#9CA3AF';
        if (r.pl === unitPlateSel) return grad(ctx, '#7C3AED', '#4C1D95');
        // a cor segue a RAZÃO, não o sinal do caixa (isso é papel do Income): no So far um carro
        // novo tem caixa negativo e plano mais negativo ainda — está à frente do plano, é verde
        const v = PCT(r); if (v == null) return '#E5E7EB';
        if (v < 0) return grad(ctx, '#F87171', '#B91C1C');
        return v >= 1 ? grad(ctx, '#34D399', '#047857') : grad(ctx, '#60A5FA', '#1D4ED8');
      };
      const pctVals = rows.map(pctVal).filter((v) => v != null);
      let pLo = Math.min(0, ...(pctVals.length ? pctVals : [0])), pHi = Math.max(100, ...(pctVals.length ? pctVals : [100]));
      const pKey = 'pct|' + [unitFleet, unitModel, unitFilter, unitSort, unitFull].join('|');
      if (unitPctLock && unitPctLock.key === pKey) { pLo = Math.min(pLo, unitPctLock.lo); pHi = Math.max(pHi, unitPctLock.hi); }
      ...
      mk('unitPct', { data: { labels, datasets: [
          Object.assign({ type: 'bar', label: '% of budget', data: rows.map(pctVal),
            backgroundColor: corPct, hoverBackgroundColor: corPct, borderRadius: 3,
            borderColor: rows.map((r) => (r.pl === unitPlateSel ? '#2E1065' : 'transparent')),
            borderWidth: rows.map((r) => (r.pl === unitPlateSel ? 1.5 : 0)) }, thin),
        ] },
        plugins: [{ id: 'pctBands', beforeDatasetsDraw(ch) {
          const { ctx, chartArea: a, scales: { y } } = ch; if (!a || !y) return;
          const y0 = y.getPixelForValue(0), y100 = y.getPixelForValue(100);
          ctx.save();
          // acima do orçado: verde; entre 0 e o orçado: neutro; abaixo de zero: vermelho
          if (y100 > a.top) { ctx.fillStyle = 'rgba(5,150,105,.045)'; ctx.fillRect(a.left, a.top, a.width, Math.min(y100, a.bottom) - a.top); }
          if (y0 < a.bottom) { ctx.fillStyle = 'rgba(220,38,38,.05)'; ctx.fillRect(a.left, Math.max(y0, a.top), a.width, a.bottom - Math.max(y0, a.top)); }
          if (y0 > a.top && y0 < a.bottom) { ctx.strokeStyle = 'rgba(31,41,55,.35)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(a.left, y0); ctx.lineTo(a.right, y0); ctx.stroke(); }
          // a linha dos 100%: o orçado de cada carro, achatado numa régua só
          if (y100 > a.top && y100 < a.bottom) {
            ctx.strokeStyle = INK_LINE(); ctx.globalAlpha = .85; ctx.lineWidth = 1.3; ctx.setLineDash([5, 4]);   // tinta do tema: no escuro a linha preta sumia
            ctx.beginPath(); ctx.moveTo(a.left, y100); ctx.lineTo(a.right, y100); ctx.stroke(); ctx.setLineDash([]);
          }
          ctx.restore();
        } },
        fleetArtPlugin,
      ...
      mk('unitRet', { data: { labels, datasets: [
          ...
          { type: 'line', label: 'Budget at each car\'s age', data: rows.map((r) => (modo15 ? null : Math.round((modoFull ? (r.budFull || 0) : r.bud) * K))),
            borderColor: 'rgba(255,255,255,.45)', borderWidth: 3, pointRadius: 0, tension: 0, order: -1, hidden: unitBudLineOff },
          { type: 'line', label: '__budget__', data: rows.map((r) => (modo15 ? null : Math.round((modoFull ? (r.budFull || 0) : r.bud) * K))),
            borderColor: 'rgba(17,24,39,.9)', borderDash: [5, 4], borderWidth: 1.3, pointRadius: 0, tension: 0, order: -2, hidden: unitBudLineOff },
        ] },
      ...
      // (plugin da arte das frotas — definido acima, usado nos dois gráficos)
      function __unitFleetArtDef() { return { id: 'unitFleetArt', beforeDatasetsDraw(ch) {
    ...
    function renderUnitBreak(rows, K, money) {
      ...
      // CONTRATO INTEIRO, não o acumulado até hoje: a pergunta é quantas recuperações um carro
      // aguenta até o FIM do ciclo — então a base é o que ele já fez + o que o orçado diz que os
      // meses restantes rendem (retFull), não o retorno parcial de um carro no M4.
      const retMed = rows.reduce((s, r) => s + (r.retFull != null ? r.retFull : r.real), 0) / n;
      let recTot = 0, recN = 0;
      ...
      box.innerHTML =
        `<div class="ub-card">` +
          ...
          // UM número principal: quantos EVENTOS de recuperação zeram o carro médio até o fim
          // do contrato. O break-even de semanas saiu do hero (continua no slider e na dispersão).
          `<div class="ub-hero ub-hero1">` +
            `<b>${f1(beRec)}</b><span>repossession events<br>zero the average car</span>` +
          `</div>` +
          `<div class="ub-row">` +
            `<div class="ub-k"><span>Full-contract return</span><b>${money(retMed)}</b><i>average car of this view</i></div>` +
            `<div class="ub-k"><span>Recovery costs</span><b class="dn">−${money(custoRec)}</b><i>recovery + repair · avg of ${recN} real cases</i></div>` +
            `<div class="ub-k"><span>Unpaid week</span><b class="dn">−${money(custoSem)}</b><i>−${money(feeMed)} avg weekly fee (fleet mix) · +${money(wearSaved)} wear the parked car skips</i></div>` +
            `<div class="ub-k ub-ev"><span>Repossession event</span><b class="dn">−${money(custoEvento)}</b><i>Recovery costs + one unpaid week</i></div>` +
          `</div>` +
          `<div class="ub-sim">` +
      ...
      costsCharts.unitScatter = new Chart(cnv.getContext('2d'), { data: {
        ...
        // o marginal escrito na PRÓPRIA linha: pílula no fim da tendência, onde o olho já está
        plugins: [{ id: 'scatTrendLbl', afterDatasetsDraw(ch) {
          const { ctx, chartArea: a, scales: { x, y } } = ch; if (!a) return;
          const xe = Math.min(xMax, x.max), ye = b0 + m * xe;
          let py = y.getPixelForValue(ye);
          py = Math.max(a.top + 14, Math.min(py, a.bottom - 14));
          const fam = getComputedStyle(document.body).fontFamily;
          const txt = (m < 0 ? '−' : '+') + money(Math.abs(m) / K) + (unitScatX === 'weeks' ? ' / unpaid week' : ' / repossession');
          ctx.save();
          ctx.font = '800 10.5px ' + fam; ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
          const tw = ctx.measureText(txt).width + 14;
          const dark = document.documentElement.classList.contains('dark');
          ctx.fillStyle = dark ? 'rgba(24,27,35,.95)' : 'rgba(255,255,255,.95)';
          ctx.strokeStyle = dark ? 'rgba(255,255,255,.25)' : 'rgba(55,65,81,.4)'; ctx.lineWidth = 1;
          ctx.beginPath(); ctx.roundRect(a.right - tw - 4, py - 10, tw, 20, 10); ctx.fill(); ctx.stroke();
          ctx.fillStyle = m < 0 ? (dark ? '#FCA5A5' : '#B91C1C') : (dark ? '#6EE7B7' : '#047857');
          ctx.fillText(txt, a.right - 11, py + .5);
          ctx.restore();
        } }],
    ...
    async function renderUnitPlate(rows) {
      ...
      // qualquer valor NEGATIVO fica vermelho, mesmo sem classe explícita (orçado, cash…)
      const chip = (t, v, cl) => { if (!cl && String(v).indexOf('−') === 0) cl = 'down'; return `<span class="up-chip${cl ? ' ' + cl : ''}"><i>${escH(t)}</i><b>${v}</b></span>`; };
      ...
      if (line === 'Insurance') {
        ...
        if (ip) {
          ...
          ip.parcelas.forEach((pc) => {
            // vencimento vazio = parcela paga à vista na assinatura do endosso (início da frota)
            const d = pc.venc ? new Date(pc.venc + 'T12:00:00') : curIni;
            const pp = Math.max(1, Math.min(PMAX, Math.floor((d - curIni) / MSd / MESd) + 1));
            // realizado a partir de 00:01 (Brasília) do DIA do vencimento — não espera o refresh
            const hojeSP = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Sao_Paulo' });
            const vencida = pc.venc ? pc.venc <= hojeSP : true;
            if (pp !== period) return;
            if (vencida) real += pc.valor / ip.carsDiv; else proj += pc.valor / ip.carsDiv;
          });
    ...
    function openInfoModal() {
      ...
      const GROUPS = [
        { id: 'rev', name: 'Revenue', dot: FAM_DOT.rev, rows: [
          ...
          { t: 'Termination fee', src: 'import_jud + projected exits + recovery % slider', upd: 'auto',
